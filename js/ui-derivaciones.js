  ...
  tbody.innerHTML = data.map(d => {
    ...
    return `
      <tr>
        <td>${d.fecha}</td>
        <td>
          <div style="font-weight:600;">${escHtml(d.paciente)}</div>
          <div style="font-size:11px;color:var(--text3);">DNI: ${escHtml(d.dni)}</div>
        </td>
        <td>
          <div style="display:flex;align-items:center;gap:6px;">
            <div style="width:8px;height:8px;border-radius:50%;background:${med?.color||'#888'};flex-shrink:0;"></div>
            <span style="font-size:13px;">${escHtml(soloApellido(d.medico))}</span>
          </div>
        </td>
        <td><span class="pill ${d.consultorio==='Palpa'?'pill-palpa':d.consultorio==='Haedo'?'pill-haedo':'pill-os'}">${d.consultorio}</span></td>
        <td>
          <div style="font-weight:600;">${escHtml(d.cirugia)}</div>
        ...
        <td style="max-width:150px;font-size:11px;color:var(--text2);">${escHtml(d.notas||"—")}</td>
        <td>
      ...
      <div class="card" style="border-top:3px solid ${med.color};">
        <div class="card-body">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
            ...
            <div style="flex:1;">
              <div style="font-weight:700;font-size:14px;">${escHtml(med.nombre)}</div>
          ...
          <div style="margin-top:10px;">
            <button class="btn btn-secondary btn-sm" style="width:100%;" onclick="filtrarPorMedico('${escJS(med.nombre)}')">Ver detalle →</button>
          </div>
        </div>
      </div>`;
  }).filter(Boolean).join('');
            ...
            <div style="flex:2;min-width:240px;">
              ...
              ${medRanking.map(([nombre, n]) => {
                ...
                return `<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px;">
                  <div style="width:8px;height:8px;border-radius:50%;background:${med?.color||'#888'};flex-shrink:0;"></div>
                  <span style="font-size:13px;flex:1;">${escHtml(soloApellido(nombre))}</span>
                  <div style="width:80px;background:var(--border);border-radius:4px;height:5px;overflow:hidden;">
...
function guardarDerivacion() {
  ...
  renderDerivKPIs();
  renderDerivaciones();
  showToast(`✓ Derivación registrada — ${escHtml(d.paciente)} · ${escHtml(d.cirugia)}`);
}
...
function abrirActualizarEstado(id) {
  ...
  document.getElementById('deriv-estado-info').innerHTML = `
    <div style="font-weight:700;font-size:14px;margin-bottom:4px;">${escHtml(d.paciente)}</div>
    <div style="font-size:12px;color:var(--text2);">${escHtml(soloApellido(d.medico))} · ${escHtml(d.cirugia)} ${d.ojo?'('+d.ojo+')':''}</div>
    <div style="font-size:12px;color:var(--text3);margin-top:4px;">${escHtml(d.os)} · <span style="${cfg.style}padding:1px 6px;border-radius:3px;">${cfg.icon} ${d.urgencia}</span></div>`;
...
function abrirPagoDerivacion(id) {
  ...
  if (sel) {
    ...
    sel.innerHTML = '<option value="">— Sin prestación / monto manual —</option>' +
      prests.map(p => `<option value="${p.id}" data-val="${p.valOS}">${escHtml(p.desc)} — ${fmt(p.valOS)}</option>`).join('');
  ...
  document.getElementById('pago-deriv-info').innerHTML = `
    <div style="font-weight:700;font-size:14px;margin-bottom:4px;">${escHtml(d.paciente)}</div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:4px;">${d.medico} · ${escHtml(d.cirugia)}${d.ojo?' ('+d.ojo+')':''}</div>
    <div style="font-size:12px;color:var(--text3);">${escHtml(d.os)} · ${d.estado}</div>
