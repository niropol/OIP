...
function _repreciarRegistros(vigencia, osFiltro, dryRun) {
  ...
  (DB.registros || []).forEach(r => {
    ...
    if (m.exenta !== undefined && m.exenta !== 'AMBIGUO') {
      ...
      if (actualEx !== m.exenta) { if (!dryRun) r.exenta = m.exenta; ivaCambiado++; }
    }
  });
...
function verificarPreciosAtenciones(osFiltro, mesFiltro) {
  ...
  (DB.registros || []).forEach(r => {
    ...
    revisadas++;
    // Clave del grupo: lo que determina el PRECIO = OS + prestación (por nombre) + PLAN + mes.
    // OJO: NO se puede agrupar por código: en OSDE varias consultas comparten el mismo código
    // (420162) con precios distintos por plan (Plan 210 ≠ Plan 510). Agrupar por código las
    // juntaría y las marcaría como "precios distintos" (falso positivo). El nombre + el plan
    // sí distinguen cada precio. Solo si hay dos precios para la MISMA prestación+plan en el
    // MISMO mes hay un reprecio a medias real.
    const clavePrest = ((r.prestacion || '').trim() || r.codigo || '(sin nombre)') + '¦' + (r.plan || '');
    const key = r.os + '||' + clavePrest + '||' + mes;
    const g = grupos[key] || (grupos[key] = { os: r.os, mes, prestacion: r.prestacion || '', codigo: r.codigo || '', valores: {}, repr: r });
...
async function confirmarActualizarPreciosOS() {
  ...
  // Preview SIN modificar (dryRun): cuántas atenciones de ESA OS desde ESE mes cambiarían.
  const prev = _repreciarRegistros(vigencia, os, true);
  if (prev.actualizadas === 0 && prev.ivaCambiado === 0) {
    if (!confirm(`No hay atenciones de ${os} desde ${mesLbl} cuyo precio o IVA cambie con los valores actuales de Configuración.\n\n¿Subir igual el nomenclador de ${os} a la nube?`)) return;
  } else {
    const partes = [];
    if (prev.actualizadas > 0) partes.push(`${prev.actualizadas} atención(es) con precio nuevo`);
    if (prev.ivaCambiado > 0)  partes.push(`${prev.ivaCambiado} con cambio de IVA`);
    if (!confirm(
      `ACTUALIZAR PRECIOS — ${os} · desde ${mesLbl}\n\n` +
      `Se van a recalcular ${partes.join(' y ')} de ${os}, desde ${mesLbl} en adelante, con los valores que tenés en Configuración.\n\n` +
