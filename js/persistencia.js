...
// guardado deja en app_meta un "syncTick" = { ts, by:sessionId }. Un poller lee ESE único
// registro cada 40 s; si el tick es más nuevo y lo escribió OTRA sesión, muestra un banner
// para traer los cambios. Así se evita el riesgo más común de multiusuario: trabajar a
// ciegas sobre datos viejos porque otra persona cargó algo y uno no lo ve hasta recargar.
const _sessionId = ((typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID() : String(Math.random()).slice(2)) + '-' + Date.now();
let _syncTickVisto = 0;     // último ts de syncTick conocido (nuestro o ya avisado)
let _syncPollTimer = null;

// Decisión PURA (testeable): ¿el tick remoto es de OTRA sesión y más nuevo que lo visto?
function _hayCambiosDeOtraSesion(remoteTick, vistoTs, miSession) {
  if (!remoteTick || typeof remoteTick.ts !== 'number') return false;
  if (remoteTick.by === miSession) return false;   // fue nuestra propia sesión
  return remoteTick.ts > (vistoTs || 0);
}
...
async function _pollSyncTick() {
  ...
  try {
    ...
    if (_hayCambiosDeOtraSesion(data.valor, _syncTickVisto, _sessionId)) {
      _syncTickVisto = data.valor.ts;   // no volver a avisar por el mismo tick
...
async function cargarDesdeNube() {
  ...
  (metas || []).forEach(m => {
    ...
    // syncTick que ya existía al cargar: registrarlo como "visto" para no auto-avisar.
    if (m.clave === 'syncTick' && m.valor && typeof m.valor.ts === 'number') _syncTickVisto = m.valor.ts;
  });
...
async function verificarSyncPrestaciones(osFiltro) {
  ...
  const { data, error } = await sb.from('app_data').select('doc_id, data').eq('coleccion', 'prestaciones');
...
async function guardarEnNube(automatico = false) {
  ...
  try {
    ...
    // 3) Meta (config, nextId, syncTick). El syncTick avisa a las OTRAS sesiones que
    //    guardamos algo; con `by:_sessionId` cada uno ignora su propio tick.
    const _tickNow = { ts: Date.now(), by: _sessionId };
    await sb.from('app_meta').upsert([
      { clave: 'config', valor: DB.config },
      { clave: 'nextId', valor: DB.nextId },
      { clave: 'syncTick', valor: _tickNow }
    ], { onConflict: 'clave' });
    _syncTickVisto = _tickNow.ts;   // nuestro propio guardado: no debe auto-avisarnos
