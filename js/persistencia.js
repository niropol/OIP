...
async function actualizarPreciosPrestaciones() {
  ...
  // AVISO de meses YA FACTURADOS cuyo total cambió: el reprecio actualizó las atenciones,
  // pero la factura emitida quedó por el monto viejo. Se informa la diferencia para emitir
  // la complementaria (no se toca la factura). Ver _diferenciasAFacturar / renderOS.
  if (typeof _diferenciasAFacturar === 'function') {
    const difs = _diferenciasAFacturar(null, vigencia);
    if (difs.length > 0) {
      const lista = difs.slice(0, 20).map(d =>
        `• ${d.os} · ${d.mesLabel}: facturado ${fmt(d.yaFacturado)} → ajustado ${fmt(d.totalAjustado)} → ` +
        (d.diferencia >= 0 ? `FALTA FACTURAR ${fmt(d.diferencia)}` : `facturado de más ${fmt(-d.diferencia)}`)
      ).join('\n');
      const extra = difs.length > 20 ? `\n…y ${difs.length - 20} más.` : '';
      alert(
        `⚠️ ${difs.length} mes(es) ya facturado(s) cambiaron de total con este reprecio.\n\n` +
        `${lista}${extra}\n\n` +
        `La factura emitida NO se modificó. Emití una factura COMPLEMENTARIA por la diferencia ` +
        `(botón "Registrar complementaria" en la pantalla de Obras Sociales).`
      );
    }
  }
...
async function guardarEnNube(automatico = false) {
  ...
  // Antes de subir, verificar contra la nube que NADIE haya guardado desde nuestra última
  // sincronización. Si otra computadora guardó, ABORTAR (no pisar) y forzar recarga. Este es
  // el candado que hace imposible que dos equipos se pisen: el que quedó viejo no puede guardar.
  ...
  try {
    ...
    try {
      ...
      const { error: eTick } = await sb.from('app_meta').upsert([{ clave: 'syncTick', valor: { token: _tok, by: _sessionId } }], { onConflict: 'clave' });
      // Solo actualizamos NUESTRO token si el sello quedó en la nube. Si falla, no lo tocamos:
      // así la próxima lectura (cloud viejo == token viejo) no se auto-bloquea por error.
      if (eTick) console.warn('syncTick no se pudo guardar (candado anti-pisada degradado):', eTick.message);
      else _syncToken = _tok;   // nuestro propio sello: no debe auto-bloquearnos
