...
function _sincronizarCajaRegistro(r) {
  ...
  const deberiaTener = (r.os === 'Particular' && ((r.partEfectivo || 0) > 0 || (r.partTransf || 0) > 0))
                    || (r.copago || 0) > 0
                    || r.os === 'SinCargo';
  // Registro viejo (sin regId) que debería tener caja: no re-sincronizar solo, avisar.
  if (!yaTiene && deberiaTener) return { legacy: true, resync: false };
  // Nada que sincronizar (p. ej. OS sin copago) y sin movimientos propios: no hacer nada.
  if (!yaTiene && !deberiaTener) return { legacy: false, resync: false };
...
function editarRegistro(id) {
  ...
  ivaBtn.textContent = r.exenta ? '✓ Exenta' : '⚡ ' + pctIVALabelOS(r.os);
...
function editarRegOSChange() {
  ...
  if (!esParticular) {
    ...
    const lbl = exenta ? '✓ Exenta' : '⚡ ' + pctIVALabelOS(os);
    btn.textContent = lbl;
...
function editarRegToggleIVA() {
  ...
  const gravLabel = '⚡ ' + pctIVALabelOS(os);
