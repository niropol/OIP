...
function renderAtenciones() {
      ...
      return `<tr>
        <td>
          <div style="display:flex;align-items:center;gap:7px;">
            ...
            <span style="font-weight:600;">${escHtml(soloApellido(m.medico))}</span>
          </div>
        </td>
        <td>${(() => {
  ...
  regs.filter(r=>r.os !== 'Particular').forEach(r => {
    ...
    const key = `${r.os}||${escHtml(r.plan)}||${exenta?'exenta':'grav'}`;
    if (!osMap[key]) osMap[key] = { os:r.os, plan:r.plan, exenta, cant:0, neto:0, iva:0, valores:new Set() };
    const g = osMap[key];
    // Sumar el neto y el IVA REALES de cada atención (subtotalNeto/ivaReg, fuente única) — así,
    // si en el mismo grupo hay valores distintos (ej. una práctica con valor especial), NO se
    // pierde. Antes se hacía cant × un solo valUnit y absorbía los valores distintos.
    g.cant += r.cantidad;
    g.neto += subtotalNeto(r);
    g.iva  += ivaReg(r);
    g.valores.add(r.valorUnit || 0);
  });
  ...
  if (osTbody) {
    ...
    osTbody.innerHTML = osRows.map(o => {
      ...
      const iva  = o.iva;
      const tot  = neto + iva;
      const valUnitShow = o.cant ? o.neto / o.cant : 0;   // promedio ponderado (marca ~ si hubo distintos)
      const mixto = o.valores.size > 1;
      ...
      return `<tr>
        ...
        <td>
          <span class="pill" style="${o.exenta?'background:var(--success-light);color:var(--success);':'background:var(--warn-light);color:var(--warn);'}">
            ${o.exenta ? '✓ Exenta' : '⚡ ' + pctIVALabelOS(o.os)}
          </span>
        </td>
        <td style="font-weight:700; text-align:center;">${o.cant}</td>
        <td class="finance-num">${fmt(valUnitShow)}${mixto?' ~':''}</td>
  ...
  if (diarioTbody) {
    ...
    diarioTbody.innerHTML = sorted.length ? sorted.map(r => {
      ...
      const osLabel = esPart ? '<span class="pill pill-particular" style="font-size:10px;">Part.</span>' : escHtml(r.os);
      const prestLabel = escHtml((r.prestacion||'').replace('oftalmológica','oftalmol.').replace('Consulta vestida','Consulta').replace('computarizada','comp.').replace('unilateral','unilat.'));
      ...
      return `<tr id="reg-row-${r.id}">
        ...
        <td style="font-weight:600;font-size:12px;">${escHtml(soloApellido(r.medico))}</td>
        <td><span class="pill ${r.consultorio==='Palpa'?'pill-palpa':'pill-haedo'}" style="font-size:10px;">${escHtml(r.consultorio)}</span></td>
        <td style="font-size:12px;">${osLabel}${r.plan?` <span style="font-size:10px;color:var(--text3);">(${escHtml(r.plan)})</span>`:''}</td>
        <td style="font-size:11px;color:var(--text3);max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${escHtml(r.prestacion||'')}">${prestLabel||'—'}</td>
        ...
        <td style="font-size:11px;text-align:center;white-space:nowrap;">
          ${esPart
            ...
            : r.os === 'CEMEPLA'
              ? `<span class="pill" style="background:var(--warn-light);color:var(--warn);font-size:10px;" title="CEMEPLA: 21% fijo · IVA ${fmt(iva)}">⚡ 21%</span>`
              : `<span class="pill" onclick="toggleExentaAtencion(${r.id})" title="Tocá para cambiar exenta/gravada${iva>0?' · IVA '+fmt(iva):''}" style="cursor:pointer;font-size:10px;${exentaReg(r,r.os)?'background:var(--success-light);color:var(--success);':'background:var(--warn-light);color:var(--warn);'}">${exentaReg(r,r.os)?'✓ Exenta':'⚡ '+pctIVALabelOS(r.os)}</span>`
          }
        </td>
        ...
        <td>
          <div style="display:flex;gap:3px;">
            ${esPart ? '' : `<button class="btn btn-secondary btn-sm" onclick="editarComentarioAtencion(${r.id})" title="${r.comentario ? 'Comentario: '+escHtml(r.comentario) : 'Agregar comentario para la presentación a la OS'}" style="${r.comentario ? 'background:var(--accent-light);' : ''}">💬</button>`}
...
// ── FUENTE ÚNICA de re-sincronización de la caja de un registro ──────────────
// Al EDITAR una atención (monto, fecha, consultorio, medio de pago…), los movimientos
// de caja que generó su alta quedaban desfasados: la caja seguía con el monto/fecha/sede
// viejos. Esta función regenera esos movimientos desde el estado ACTUAL del registro,
// espejando exactamente lo que crea guardarAtencion (mismos concepto/origen/forma).
//
// SEGURIDAD anti-duplicados: solo regenera si el registro YA tiene movimientos ligados
// por regId (los creó el alta nueva). Un registro VIEJO sin regId (cuya caja se vincula
// por coincidencia de texto) NO se toca automáticamente para no duplicar: se devuelve
// { legacy:true } para avisar y que se corrija a mano. El pago del honorario en efectivo
// se preserva según la marca honorarioPagadoEfectivo (no se vuelve a preguntar en la edición).
function _sincronizarCajaRegistro(r) {
  const yaTiene = DB.cajaChica.some(m => m.regId === r.id) || DB.movimientos.some(m => m.regId === r.id);
  // Solo Particular (con montos) y SinCargo pudieron tener caja ligada por TEXTO en registros
  // viejos (pre-regId): esos, si no tienen movimientos por regId, se marcan legacy y se avisan
  // (no se tocan solos para no duplicar). El copago SIEMPRE se ligó por regId, así que un copago
  // recién agregado en la edición se puede crear desde cero sin riesgo de duplicar.
  const esLegacySensible = (r.os === 'Particular' && ((r.partEfectivo || 0) > 0 || (r.partTransf || 0) > 0))
                        || r.os === 'SinCargo';
  if (!yaTiene && esLegacySensible) return { legacy: true, resync: false };
  // Sin movimientos propios y sin copago que crear (p. ej. OS común): no hacer nada.
  if (!yaTiene && !((r.copago || 0) > 0)) return { legacy: false, resync: false };

  // Tiene movimientos propios (por regId): borrarlos y recrearlos desde el estado actual.
  DB.cajaChica   = DB.cajaChica.filter(m => m.regId !== r.id);
  DB.movimientos = DB.movimientos.filter(m => m.regId !== r.id);

  const ape = soloApellido(r.medico);
  const { fecha, consultorio } = r;

  if (r.os === 'Particular') {
    const efMonto = (r.partEfectivo || 0) * (r.partEfVal || 0);
    const trMonto = (r.partTransf  || 0) * (r.partTrVal || 0);
    // Efectivo → caja chica del consultorio
    if (efMonto > 0) {
      DB.cajaChica.push({
        id: DB.nextId++, fecha, consultorio, tipo: 'Ingreso',
        concepto: `Particular efectivo — ${ape}`, origen: 'Efectivo', monto: efMonto, regId: r.id,
      });
      // Honorario efectivo ya pagado en el acto: se conserva la decisión original.
      if (r.honorarioPagadoEfectivo) {
        DB.cajaChica.push({
          id: DB.nextId++, fecha, consultorio, tipo: 'Egreso',
          concepto: `Pago honorario efectivo — ${ape} — ${fecha}`, origen: 'Honorario', monto: efMonto / 2, regId: r.id,
        });
      }
    } else if (r.honorarioPagadoEfectivo) {
      // Ya NO hay parte en efectivo (ej. se editó a transferencia): el flag "honorario
      // pagado en efectivo" queda sin sentido. Limpiarlo evita que una edición futura de
      // vuelta a efectivo recree el egreso sin preguntar, afirmando un pago que no ocurrió.
      delete r.honorarioPagadoEfectivo;
    }
    // Transferencia → Caja (banco)
    if (trMonto > 0) {
      DB.movimientos.unshift({
        id: DB.nextId++, fecha, consultorio, tipo: 'Ingreso',
        desc: `Particular transferencia — ${ape}`, monto: trMonto, saldo: 0, origen: 'Particular', regId: r.id,
      });
    }
  }

  // Copago (adelanto o complementario): efectivo → caja chica; transferencia → banco.
  if ((r.copago || 0) > 0) {
    const etTipo   = r.copagoTipo === 'complementario' ? 'complementario' : 'adelanto';
    const prestDesc = (r.prestacion || '').slice(0, 28);
    if (r.copagoMedio === 'Efectivo' || !r.copagoMedio) {
      DB.cajaChica.push({
        id: DB.nextId++, fecha, consultorio, tipo: 'Ingreso',
        concepto: `Copago ${r.os} (efectivo, ${etTipo}) — ${prestDesc}`, origen: 'Copago', monto: r.copago, regId: r.id,
      });
    } else {
      DB.movimientos.unshift({
        id: DB.nextId++, fecha, consultorio, tipo: 'Ingreso',
        desc: `Copago ${r.os} (transferencia, ${etTipo}) — ${prestDesc}`, monto: r.copago, saldo: 0, origen: 'Copago', regId: r.id,
      });
    }
  }

  // SinCargo: ingreso + egreso de $0,1 (neto cero), ambos ligados al registro.
  if (r.os === 'SinCargo') {
    const nomPac = r.paciente || `${r.apellido || ''}, ${r.nombre || ''}`.replace(/^, |, $/, '');
    DB.movimientos.unshift({
      id: DB.nextId++, fecha, consultorio, tipo: 'Ingreso',
      desc: `SinCargo — ${nomPac} (paga ${ape})`, monto: 0.1, saldo: 0, origen: 'SinCargo', regId: r.id,
    });
    DB.movimientos.unshift({
      id: DB.nextId++, fecha, consultorio, tipo: 'Egreso',
      desc: `SinCargo — pago a ${ape} (${nomPac})`, monto: 0.1, saldo: 0, origen: 'SinCargo', regId: r.id,
    });
  }

  marcarCambios('cajaChica');
  marcarCambios('movimientos');
  return { legacy: false, resync: true };
}
...
function eliminarRegistro(id) {
  ...
  if (todosMovs.length > 0) {
    const detalle = todosMovs.map(m => `• ${m.tipo} · ${escHtml(m.concepto || m.desc)} · ${fmt(m.monto)}`).join('\n');
...
// Cambia el IVA (exenta ↔ gravada) de UNA atención directo desde la lista de Atenciones.
// A propósito NO chequea regBloqueado (liquidación cerrada): el IVA no afecta lo que cobra el
// médico, así que se puede corregir sin reabrir la liquidación. Si ya hay factura emitida de
// esa OS/mes, _confirmarCambioExencion avisa que hay que regenerarla. Particular no lleva IVA;
// CEMEPLA es fija 21%.
function toggleExentaAtencion(regId) {
  const r = DB.registros.find(x => x.id === regId);
  if (!r) return;
  if (r.os === 'Particular') { showToast('Los particulares no llevan IVA.'); return; }
  if (r.os === 'CEMEPLA') { showToast('CEMEPLA siempre va gravada al 21%.'); return; }
  const mes = (r.fecha || '').slice(0, 7);
  if (typeof _confirmarCambioExencion === 'function' && !_confirmarCambioExencion(r.os, mes, 'esta atención')) return;
  const nuevo = !exentaReg(r, r.os);
  r.exenta = nuevo;
  marcarCambios('registros');
  renderAtenciones();
  if (typeof initDashboard === 'function') initDashboard();
  showToast(nuevo ? '✓ Atención marcada como EXENTA' : '✓ Atención marcada como GRAVADA ' + pctIVALabelOS(r.os));
}

// Editar el comentario/observación de una atención SIN pasar por el modal completo.
// A diferencia de editarRegistro, esto NO exige la liquidación abierta: el comentario es una
// nota que aparece en la presentación a la OS y NO afecta el honorario del médico (igual que el
// toggle de IVA). Suele necesitarse justo cuando el mes ya está cerrado y se arman las facturas.
function editarComentarioAtencion(regId) {
  const r = DB.registros.find(x => x.id === regId);
  if (!r) return;
  if (r.os === 'Particular') { showToast('El comentario solo se usa en la presentación a la OS.'); return; }
  const actual = r.comentario || '';
  const val = prompt('Observación / comentario (aparece en la presentación a la OS):', actual);
  if (val === null) return;                 // canceló
  const limpio = val.trim();
  if (limpio) r.comentario = limpio;
  else delete r.comentario;
  if (limpio === actual.trim()) return;     // no cambió nada
  marcarCambios('registros');
  renderAtenciones();
  showToast(limpio ? '💬 Comentario guardado' : '💬 Comentario borrado');
}

// Opciones <option> para un select simple (consultorio/OS), con el valor actual seleccionado.
// Si el valor actual no está en la lista (ej: consultorio inactivo, OS dada de baja), se
// agrega igual para no perderlo al editar.
function _optionsListEdit(items, current) {
  const lista = [...items];
  if (current && !lista.includes(current)) lista.unshift(current);
  return lista.map(v => `<option value="${escHtml(v)}"${v === current ? ' selected' : ''}>${escHtml(v)}</option>`).join('');
}

// Opciones de prestación para el modal de EDICIÓN: TODAS las prestaciones de la OS
// (consulta + estudio + práctica) con sus data-attrs (valor/IVA/categoría/código), + "otra
// a mano". La primera opción SIEMPRE es "(mantener actual)" con los valores actuales del
// registro, seleccionada por defecto salvo que la prestación actual coincida con una del
// nomenclador. Así, guardar sin tocar el selector nunca cambia la prestación.
function _optionsPrestacionesEdit(os, r) {
  const listar = (typeof getConsultasDeOS === 'function')
    ? [...getConsultasDeOS(os), ...getEstudiosDeOS(os), ...getPracticasDeOS(os)]
    : (DB.prestaciones || []).filter(p => p.os === os);
  const exAttr = p => (typeof exentaPrestacion === 'function' ? exentaPrestacion(p) : !!p.exenta) ? '0' : '1'; // 1 = gravada
  const catDe  = p => (typeof categoriaPrestacion === 'function') ? categoriaPrestacion(p) : (p.categoria || 'practica');
  const actualDesc = (r.prestacion || '').trim();
  const matchNomen = actualDesc && listar.some(p => (p.desc || '').trim() === actualDesc);
  // Opción "(mantener actual)": conserva la prestación/valor/IVA/categoría/código actuales.
  const optActual = `<option value="__actual__" data-val="${r.valorUnit || 0}" data-exenta="${r.exenta ? '0' : '1'}" data-cat="${escHtml(r.categoria || 'practica')}" data-codigo="${escHtml(r.codigo || '')}" data-desc="${escHtml(actualDesc)}"${matchNomen ? '' : ' selected'}>${actualDesc ? escHtml(actualDesc) + ' (actual)' : '— mantener —'}</option>`;
  const optsNomen = listar
    .sort((a, b) => cmpAlfa(a.desc, b.desc))
    .map(p => {
      const sel = matchNomen && (p.desc || '').trim() === actualDesc ? ' selected' : '';
      return `<option value="${p.id}" data-val="${p.valOS}" data-exenta="${exAttr(p)}" data-cat="${catDe(p)}" data-codigo="${escHtml(p.codigo || '')}" data-desc="${escHtml(p.desc || '')}"${sel}>[${escHtml(p.codigo || '')}] ${escHtml(p.desc || '')}${p.valOS > 0 ? ' — ' + fmt(p.valOS) : ''}</option>`;
    }).join('');
  // "Otra" no cambia la categoría (data-cat vacío → se preserva la del registro).
  const exOS = (typeof getExentaForOS === 'function' && getExentaForOS(os)) ? '0' : '1';
  const optOtra = `<option value="__otra__" data-val="0" data-exenta="${exOS}" data-cat="" data-codigo="" data-desc="">➕ Otra prestación (nombre a mano)…</option>`;
  return optActual + optsNomen + optOtra;
}

// Refresca el botón de IVA del modal de edición según el hidden edit-reg-iva-val (1 = exenta).
function _editRegSyncIvaBtn() {
  ...
  const exenta = hidden.value === '1';
  btn.textContent = exenta ? '✓ Exenta' : '⚡ ' + pctIVALabelOS(os);
...
function editarRegistro(id) {
  ...
  document.getElementById('edit-reg-medico').innerHTML = optionsMedicos(r.medico);
  // Consultorio y OS: dinámicos desde la base (no hardcodeados) — así aparecen todas las OS
  // reales (incluida SinCargo y cualquier alta nueva) y todos los consultorios activos.
  document.getElementById('edit-reg-consultorio').innerHTML = _optionsListEdit(getConsultoriosList(), r.consultorio);
  document.getElementById('edit-reg-os').innerHTML = _optionsListEdit([...getOSList(), 'Particular'], r.os);
  ...
  // Selector de prestación (solo OS) + ocultos de código/categoría
  const prestSel = document.getElementById('edit-reg-prestacion');
  if (prestSel && !esParticular) prestSel.innerHTML = _optionsPrestacionesEdit(r.os, r);
  const otraInp = document.getElementById('edit-reg-prest-otra');
  if (otraInp) { otraInp.value = ''; otraInp.style.display = 'none'; }
  const filtroInp = document.getElementById('edit-reg-prest-filtro');
  if (filtroInp) filtroInp.value = '';
  const codEl = document.getElementById('edit-reg-codigo');       if (codEl) codEl.value = r.codigo || '';
  const catEl = document.getElementById('edit-reg-categoria');    if (catEl) catEl.value = r.categoria || '';
  ...
  const _cmt = document.getElementById('edit-reg-comentario'); if (_cmt) _cmt.value = r.comentario || '';

  // Copago / coseguro
  const copChk = document.getElementById('edit-reg-tiene-copago');
  const tieneCop = (r.copago || 0) > 0;
  if (copChk) copChk.checked = tieneCop;
  const copMonto = document.getElementById('edit-reg-copago-monto'); if (copMonto) copMonto.value = r.copago || 0;
  const copMedio = document.getElementById('edit-reg-copago-medio'); if (copMedio) copMedio.value = r.copagoMedio || 'Efectivo';
  const copTipo  = document.getElementById('edit-reg-copago-tipo');  if (copTipo)  copTipo.value  = r.copagoTipo || 'adelanto';
  const copFields = document.getElementById('edit-reg-copago-fields'); if (copFields) copFields.style.display = tieneCop ? '' : 'none';
...
function editarRegOSChange() {
  ...
  // Repoblar el selector de prestación con las de la OS nueva (sin prestación previa, para no
  // arrastrar una de otra OS). El valor/IVA se recalculan según la prestación elegida.
  const prestSel = document.getElementById('edit-reg-prestacion');
  if (prestSel) {
    prestSel.innerHTML = _optionsPrestacionesEdit(os, { prestacion: '', valorUnit: 0, exenta: getExentaForOS(os), categoria: '', codigo: '' });
    const filtroInp = document.getElementById('edit-reg-prest-filtro');
    if (filtroInp) filtroInp.value = '';
    // Valor por defecto de la OS NUEVA: sin esto, "— mantener —" dejaba el precio de la OS vieja
    // (stale). Si después eligen una prestación puntual, editarRegPrestChange lo pisa.
    document.getElementById('edit-reg-valunit').value = valorConsultaOS(os);
    document.getElementById('edit-reg-iva-val').value = getExentaForOS(os) ? '1' : '0';
    _editRegSyncIvaBtn();
    editarRegPrestChange();   // sincroniza valor/IVA/categoría con la opción seleccionada
  } else {
    // Sin selector de prestación (compat): fallback al valor de consulta de la OS
    document.getElementById('edit-reg-valunit').value = valorConsultaOS(os);
    document.getElementById('edit-reg-iva-val').value = getExentaForOS(os) ? '1' : '0';
    _editRegSyncIvaBtn();
  }
...
// Al elegir una prestación en la edición: autocompleta valor + IVA + categoría + código
// (igual que en la carga). "Otra" muestra el campo de nombre a mano y no pisa la categoría.
function editarRegPrestChange() {
  const sel = document.getElementById('edit-reg-prestacion');
  if (!sel || !sel.options.length) return;
  const opt = sel.options[sel.selectedIndex];
  const otraInp = document.getElementById('edit-reg-prest-otra');
  const esOtra = opt?.value === '__otra__';
  if (otraInp) otraInp.style.display = esOtra ? '' : 'none';
  const cat = opt?.getAttribute('data-cat') || '';
  if (cat) { const catEl = document.getElementById('edit-reg-categoria'); if (catEl) catEl.value = cat; }
  const codEl = document.getElementById('edit-reg-codigo'); if (codEl) codEl.value = opt?.getAttribute('data-codigo') || '';
  if (esOtra) { if (otraInp) otraInp.focus(); return; }   // nombre + valor a mano
  if (opt?.value === '__actual__') return;                // mantener: no pisar valor/IVA
  const val = parseFloat(opt?.getAttribute('data-val')) || 0;
  if (val > 0) document.getElementById('edit-reg-valunit').value = val;
  const gravada = opt?.getAttribute('data-exenta') === '1';
  document.getElementById('edit-reg-iva-val').value = gravada ? '0' : '1';
  _editRegSyncIvaBtn();
}

function editarRegToggleCopago() {
  const on = document.getElementById('edit-reg-tiene-copago')?.checked;
  const f = document.getElementById('edit-reg-copago-fields');
...
function guardarEdicionRegistro() {
  ...
  // Un particular NO se puede reasignar de OS desde el form (su fila de OS está oculta y el
  // desplegable edit-reg-os ni siquiera tiene la opción "Particular"). Si leyéramos ese select
  // para un particular, r.os quedaría en '' y la atención se corrompería (facturación/honorarios
  // dejarían de reconocerla como particular). Por eso solo se toma el OS del select cuando NO
  // es particular.
  const eraParticular = r.os === 'Particular';
  ...
  if (!eraParticular) r.os = document.getElementById('edit-reg-os').value;
  ...
  // Comentario/observación (aparece en la presentación de la OS). Si queda vacío, se quita
  // el campo para no dejar la clave con string vacío.
  const _cmt = document.getElementById('edit-reg-comentario');
  if (_cmt) { const v = _cmt.value.trim(); if (v) r.comentario = v; else delete r.comentario; }

  // Prestación (nombre + código + categoría), editable como en la carga. El default
  // "(mantener actual)" conserva lo que había; una del nomenclador o "otra" lo cambian.
  const prestSel = document.getElementById('edit-reg-prestacion');
  if (!eraParticular && prestSel && prestSel.options.length) {
    const opt = prestSel.options[prestSel.selectedIndex];
    if (opt?.value === '__otra__') {
      const nombre = (document.getElementById('edit-reg-prest-otra')?.value || '').trim();
      if (nombre) { r.prestacion = nombre; r.codigo = ''; }   // sin nombre: no toca la prestación
    } else {
      const nombre = opt?.getAttribute('data-desc') || '';
      r.prestacion = nombre;
      r.codigo = opt?.getAttribute('data-codigo') || '';
    }
    const cat = document.getElementById('edit-reg-categoria')?.value;
    if (cat) r.categoria = cat;
  }

  // Copago / coseguro (adelanto o complementario). El caja-resync (más abajo) recrea el
  // movimiento correspondiente. Si se destilda, se limpia.
  if (!eraParticular) {
    const tieneCop = document.getElementById('edit-reg-tiene-copago')?.checked;
    if (tieneCop) {
      r.copago      = parseFloat(document.getElementById('edit-reg-copago-monto')?.value) || 0;
      r.copagoMedio = document.getElementById('edit-reg-copago-medio')?.value || 'Efectivo';
      r.copagoTipo  = document.getElementById('edit-reg-copago-tipo')?.value || 'adelanto';
      if (r.copago <= 0) r.copago = 0;
    } else {
      r.copago = 0;
    }
  }

  // Re-sincronizar la caja: los movimientos que generó el alta (efectivo, honorario,
  // copago, transferencia, SinCargo) deben reflejar el nuevo monto/fecha/sede/medio.
  const sync = _sincronizarCajaRegistro(r);

  closeModal('modal-edit-registro');
  renderAtenciones();
  if (sync.resync) { if (typeof renderCajaChica === 'function') renderCajaChica(); if (typeof renderFinanzas === 'function') renderFinanzas(); }
  marcarCambios('registros');
  if (sync.legacy) {
    showToast('✓ Registro actualizado — ⚠️ revisá la caja: esta atención tiene movimientos viejos que no se re-sincronizan solos.');
  } else {
    showToast(`✓ Registro actualizado — ${soloApellido(r.medico)} · ${r.os} · ${r.fecha}`);
  }
}
...
function makeIVABadge(exenta, rowId, os) {
  const label = exenta ? '✓ Exenta' : '⚡ ' + pctIVALabelOS(os);
  const style = exenta
    ? 'background:var(--success-light);color:var(--success);cursor:pointer;'
    : 'background:var(--warn-light);color:var(--warn);cursor:pointer;';
...
function toggleIVARow(rowId) {
  ...
  badge.textContent = ahora ? '✓ Exenta' : '⚡ ' + pctIVALabelOS(os);
...
// Secuencia monotónica para ids de fila de carga masiva. Date.now() colisionaba cuando se
// creaban dos filas en el mismo milisegundo (al duplicar, o al agregar dos veces rápido):
// dos filas con el mismo id hacían que eliminar/togglear IVA actuara sobre la fila equivocada.
let _cmRowSeq = 0;

// ── TABLA CONSULTAS ──────────────────────────────────────────────────────────
function agregarFilaConsulta() {
  const tbody = document.getElementById('cm-consultas-tbody');
  const id = 'ccrow_' + (++_cmRowSeq);
  const os0 = CM_OS_LIST[0];
  const consultas0 = getConsultasDeOS(os0).sort((a, b) => cmpAlfa(a.desc, b.desc));
  ...
  const opts0 = consultas0.length
    ? consultas0.map(p => `<option value="${p.id}" data-val="${p.valOS}" data-exenta="${exentaPrestacion(p)?'1':'0'}" data-os="${os0}">[${escHtml(p.codigo)}] ${escHtml(p.desc)} — ${fmt(p.valOS)}</option>`).join('')
  ...
  tbody.insertAdjacentHTML('beforeend', `
    <tr id="${id}">
      ...
      <td style="text-align:center; padding:2px;">
        <button onclick="duplicarFilaCM('${id}')" title="Duplicar fila (para dividir el IVA: ej. 2 exentos + 1 gravado)" style="background:none;border:none;cursor:pointer;color:var(--accent2);font-size:14px;">⧉</button>
...
function onCMConsultaOSChange(sel, rowId) {
  ...
  const os = sel.value;
  const consultas = getConsultasDeOS(os).sort((a, b) => cmpAlfa(a.desc, b.desc));
  ...
  if (prestSel) {
    prestSel.innerHTML = consultas.length
      ? consultas.map(p => `<option value="${p.id}" data-val="${p.valOS}" data-exenta="${exentaPrestacion(p)?'1':'0'}" data-os="${os}">[${escHtml(p.codigo)}] ${escHtml(p.desc)} — ${fmt(p.valOS)}</option>`).join('')
...
// Duplica una fila de carga masiva (consulta o prestación) copiando OS, prestación, plan,
// cantidad, valor e IVA. Sirve para DIVIDIR el IVA de un grupo: ej. cargar "3 OSDE 310"
// exentos, duplicar, y en la copia poner cantidad 1 y tocar el badge a gravado → quedan
// 2 exentos + 1 gravado (dos registros), que hoy no se podía en una sola fila.
function duplicarFilaCM(rowId) {
  const src = document.getElementById(rowId);
  if (!src) return;
  const esConsulta = src.parentElement && src.parentElement.id === 'cm-consultas-tbody';
  // Capturar la fila origen
  const os       = src.querySelector('select')?.value || '';
  const prestSel = src.querySelector('.cm-prest-sel');
  const prestVal = prestSel ? prestSel.value : null;
  const plan     = src.querySelector('.cm-plan-input')?.value || '';
  const cant     = src.querySelector('.cm-cant-input')?.value || '0';
  const val      = src.querySelector('.cm-val-input')?.value || '0';
  const exenta   = src.querySelector('.iva-hidden')?.value === '1';
  // Crear la fila nueva (queda al final del tbody correspondiente)
  if (esConsulta) agregarFilaConsulta(); else agregarFilaPrestacion();
  const tbody = document.getElementById(esConsulta ? 'cm-consultas-tbody' : 'cm-prest-tbody');
  const nueva = tbody && tbody.lastElementChild;
  if (!nueva) return;
  // OS → repuebla el desplegable de prestaciones de la fila nueva
  const osSel = nueva.querySelector('select');
  if (osSel && os) { osSel.value = os; osSel.dispatchEvent(new Event('change')); }
  // Prestación → dispara el cambio de valor/IVA de esa opción
  const nPrest = nueva.querySelector('.cm-prest-sel');
  if (nPrest && prestVal != null && [...nPrest.options].some(o => o.value === prestVal)) {
    nPrest.value = prestVal; nPrest.dispatchEvent(new Event('change'));
  }
  // Copiar plan / cantidad / valor de la fila origen (después del change, que los pisa)
  const nPlan = nueva.querySelector('.cm-plan-input'); if (nPlan) nPlan.value = plan;
  const nCant = nueva.querySelector('.cm-cant-input'); if (nCant) nCant.value = cant;
  const nVal  = nueva.querySelector('.cm-val-input');  if (nVal)  nVal.value = val;
  // Igualar el IVA al de la fila origen (si el change lo dejó distinto)
  const nHidden = nueva.querySelector('.iva-hidden');
  if (nHidden && (nHidden.value === '1') !== exenta) toggleIVARow(nueva.id);
  recalcCM();
}
...
function agregarFilaPrestacion() {
  ...
  const id = 'cprow_' + (++_cmRowSeq);
  const os0 = CM_OS_LIST[0];
  const prests0 = getPrestacionesSinConsulta(os0).sort((a, b) => cmpAlfa(a.desc, b.desc));
  ...
  const opts0 = prests0.length
    ? prests0.map(p => `<option value="${p.id}" data-val="${p.valOS}" data-exenta="${exentaPrestacion(p)?'1':'0'}" data-os="${os0}">[${escHtml(p.codigo)}] ${escHtml(p.desc)} — ${fmt(p.valOS)}</option>`).join('')
...
function onCMPrestOSChange(sel, rowId) {
  const row = document.getElementById(rowId); if (!row) return;
  const os = sel.value;
  const prests = getPrestacionesSinConsulta(os).sort((a, b) => cmpAlfa(a.desc, b.desc));
  ...
  if (prestSel) {
    prestSel.innerHTML = prests.length
      ? prests.map(p => `<option value="${p.id}" data-val="${p.valOS}" data-exenta="${exentaPrestacion(p)?'1':'0'}" data-os="${os}">[${escHtml(p.codigo)}] ${escHtml(p.desc)} — ${fmt(p.valOS)}</option>`).join('')
...
function getExentaForOS(os) {
  if (os === 'CEMEPLA') return false;  // CEMEPLA tiene IVA 21% (no exenta), no configurable
  // Data-driven con fallback: si la ficha de la OS define exentaDefault (true/false), manda;
  // si no, la lista histórica (CoberMed/Medical's gravadas, el resto exentas). Así una OS
  // nueva o que cambió su forma de facturar fija su condición de IVA desde el alta.
  const o = (DB.obrasSociales || []).find(x => x.nombre === os);
  if (o && typeof o.exentaDefault === 'boolean') return o.exentaDefault;
  return !OS_GRAVADAS_SIEMPRE.has(os);
}

function getIVALabel(os, exenta) {
  if (exenta) return '✓ Exenta';
  return '⚡ ' + pctIVALabelOS(os);   // data-driven (21% CEMEPLA / lo que fije la ficha)
}
  ...
  const nombresHTML = particularesNombres.filter(p=>!p.os).length > 0
    ? `<ul style="margin:6px 0 0;padding-left:18px;font-size:12px;">${
        particularesNombres.filter(p=>!p.os).map(p => {
          ...
          return `<li>${escHtml(p.nombre)}${badge}</li>`;
        }).join('')
      }</ul>` : '';
    ...
    confirmDiv.innerHTML = `
      <div style="background:var(--surface);border-radius:14px;padding:24px;max-width:420px;width:92%;box-shadow:0 20px 60px rgba(0,0,0,.3);border-top:4px solid var(--warn);">
        ...
        <div style="font-size:13px;color:var(--text2);margin-bottom:14px;">
          <strong>${escHtml(soloApellido(medico))}</strong> ya tiene
