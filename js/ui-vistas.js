...
// Fecha corta "DD/MM HH:MM" (hora Argentina) para notas y respuestas.
function _fechaNotaCorta() {
  const now = new Date();
  return `${hoyISO().slice(8,10)}/${hoyISO().slice(5,7)} ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
}

// Pizarrón como LISTA DE TAREAS: cada tarea se puede marcar realizada (checkbox) y responder.
function renderNotas() {
  const cont = document.getElementById('notas-lista');
  if (!cont) return;
  // Preservar respuestas a medio escribir (el render recrea los inputs) y el foco, para que
  // marcar/responder en una tarea no borre lo que se estaba tipeando en otra.
  const enCurso = {};
  cont.querySelectorAll('input[id^="nota-resp-"]').forEach(inp => { if (inp.value) enCurso[inp.id] = inp.value; });
  const ael = document.activeElement;
  const focoId = ael && ael.id && ael.id.indexOf('nota-resp-') === 0 ? ael.id : null;
  const notas = DB.notas || [];
  if (notas.length === 0) {
    cont.innerHTML = '<span style="font-size:11px; color:#b45309; opacity:.7;">Sin tareas — lo que agregues acá lo ven todos los usuarios.</span>';
    return;
  }
  cont.innerHTML = notas.map(n => {
    const hecho = !!n.hecho;
    const respuestas = (n.respuestas || []).map(r => `
      <div style="font-size:12px; color:#78350f; padding:2px 0 2px 24px; border-top:1px dashed #fcd34d;">
        <span style="color:#b45309;">↳</span> ${escHtml(r.texto)}
        <span style="font-size:10px; color:#b45309; opacity:.7; margin-left:4px; white-space:nowrap;">${escHtml(r.fecha || '')}</span>
      </div>`).join('');
    return `
    <div style="background:#fef3c7; border:1px solid #fcd34d; border-radius:8px; padding:8px 10px;">
      <div style="display:flex; align-items:flex-start; gap:8px;">
        <input type="checkbox" ${hecho ? 'checked' : ''} onchange="toggleNotaHecha(${n.id})" title="Marcar realizada" style="width:auto; margin-top:3px; cursor:pointer;">
        <div style="flex:1; min-width:0;">
          <div style="font-size:13px; color:#78350f; word-break:break-word;${hecho ? ' text-decoration:line-through; opacity:.55;' : ''}">${escHtml(n.texto)}</div>
          <div style="font-size:10px; color:#b45309; opacity:.7;">${escHtml(n.fecha || '')}</div>
        </div>
        <button onclick="eliminarNota(${n.id})" title="Borrar tarea"
          style="background:none; border:none; cursor:pointer; color:#b45309; font-size:13px; padding:0 2px;">✕</button>
      </div>
      ${respuestas}
      <div style="display:flex; gap:6px; margin-top:6px; padding-left:24px;">
        <input type="text" id="nota-resp-${n.id}" placeholder="Responder…"
          onkeydown="if(event.key==='Enter') responderNota(${n.id})"
          style="flex:1; min-width:0; font-size:12px; padding:4px 8px; border:1px solid #fcd34d; border-radius:6px; background:#fffbeb;">
        <button class="btn btn-sm" style="background:#f59e0b; color:#fff; border:none; white-space:nowrap;" onclick="responderNota(${n.id})">Responder</button>
      </div>
    </div>`;
  }).join('');
  // Restaurar lo tipeado y el foco tras el redibujo.
  Object.keys(enCurso).forEach(id => { const el = document.getElementById(id); if (el) el.value = enCurso[id]; });
  if (focoId) { const el = document.getElementById(focoId); if (el) { el.focus(); try { el.setSelectionRange(el.value.length, el.value.length); } catch (e) {} } }
}

function agregarNota() {
  const inp = document.getElementById('nota-nueva');
  const texto = (inp?.value || '').trim();
  if (!texto) { showToast('⚠️ Escribí la tarea primero'); return; }
  if (!DB.notas) DB.notas = [];
  DB.notas.unshift({ id: DB.nextId++, texto, fecha: _fechaNotaCorta(), hecho: false, respuestas: [] });
  if (inp) inp.value = '';
  marcarCambios('notas');
  renderNotas();
}

// Marca/desmarca una tarea como realizada (queda tachada, no se borra).
...
// Agrega una respuesta a una tarea (visible para todo el equipo).
function responderNota(id) {
  const inp = document.getElementById('nota-resp-' + id);
  const texto = (inp?.value || '').trim();
  if (!texto) return;
  const n = (DB.notas || []).find(x => x.id === id);
  if (!n) return;
  if (!n.respuestas) n.respuestas = [];
  n.respuestas.push({ texto, fecha: _fechaNotaCorta() });
  if (inp) inp.value = '';   // se envió: vaciar ANTES del render para que no se re-preserve
  marcarCambios('notas');
  renderNotas();
}
...
function initDashboard() {
  ...
  // Etiqueta del mes desde hoyISO (hora Argentina), no desde new Date() (TZ del navegador):
  // así el rótulo coincide siempre con el mes que se filtra.
  const [_anio, _mesNum] = mesPrefijo.split('-');
  const mesLabel = MESES[+_mesNum] + ' ' + _anio;
  ...
  const pillsConsultorio = lista => lista.filter(c=>c.cant>0 || ['Palpa','Haedo'].includes(c.nombre))
    .map(c => `<span class="pill ${pillClaseConsultorio(c.nombre)}" style="font-size:14px; margin-left:4px;"${1}>${escHtml(c.nombre)} ${c.cant}</span>`).join('');
  ...
  // Médicos activos hoy
  const medicoCount = {};
  const medicoConsult = {};   // consultorio(s) REALES donde atendió hoy cada médico (con conteo)
  regsHoy.forEach(r => {
    const tot = totalConsultasReg(r);
    if (tot > 0) {
      medicoCount[r.medico] = (medicoCount[r.medico]||0) + tot;
      const porCons = medicoConsult[r.medico] = medicoConsult[r.medico] || {};
      if (r.consultorio) porCons[r.consultorio] = (porCons[r.consultorio]||0) + tot;
    }
  });
  ...
  if (list) {
    ...
    list.innerHTML = sorted.length ? sorted.map(([nombre, count]) => {
      ...
      return `<div style="display:flex; align-items:center; gap:10px; margin-bottom:10px;">
        ...
        <div style="flex:1;">
          <div style="font-size:13px;font-weight:600;margin-bottom:3px;">${escHtml(nombre)}</div>
        ...
        ${(() => {
          // Consultorio(s) REAL(es) de las atenciones de hoy de este médico (no la ficha).
          // Si atendió en más de uno, se muestran todos con su conteo para no confundir.
          const porCons = medicoConsult[nombre] || {};
          const entries = Object.entries(porCons).sort((a,b)=>b[1]-a[1]);
          if (entries.length === 0) return `<span class="pill pill-os" style="font-size:10px;">${med?.consultorio||'—'}</span>`;
          return entries.map(([c,n]) => `<span class="pill ${pillClaseConsultorio(c)}" style="font-size:10px;">${c}${entries.length>1?' '+n:''}</span>`).join(' ');
        })()}
    ...
    timeline.innerHTML = recents.map(r=>{
      ...
      return `<li class="timeline-item">
        ...
        <div class="timeline-content">
          <div class="timeline-title">${escHtml(soloApellido(r.medico))} — ${escHtml(r.os)}${r.plan?' ('+r.plan+')':''} · ${tot} consulta${tot!==1?'s':''}</div>
          <div class="timeline-meta">${escHtml(r.consultorio)} · ${r.fecha}</div>
        </div>
      </li>`;
  ...
  if (da) {
    ...
    da.innerHTML = activas.length ? activas.map(al=>`
      <div style="display:flex;align-items:flex-start;gap:8px;padding:10px 0;border-bottom:1px solid var(--border);">
        ...
        <div>
          <div style="font-size:13px;font-weight:600;">${escHtml(al.titulo)}</div>
...
function renderEstadisticas() {
  ...
  if (rankEl) rankEl.innerHTML = sorted.map(([nombre, count], i) => {
    ...
    return `<div style="display:flex; align-items:center; gap:8px; margin-bottom:10px;">
      ...
      <div style="flex:1;">
        <div style="font-size:13px; font-weight:500; margin-bottom:3px;">${escHtml(nombre)}</div>
