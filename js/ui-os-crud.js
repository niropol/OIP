...
function generarPreliqOS() {
  ...
  Object.entries(osGroups).forEach(([osName, regs]) => {
    ...
    // Desglose a mostrar: neto de copago si hay coseguro a cuenta (el coseguro hereda el IVA
    // de la prestación), si no, el bruto. Suma = total a facturar en ambos casos.
    const _hayCop = esCopagoAdelanto(osName) && totalAdelanto > 0;
    const dispExento  = _hayCop ? t.netoExentoAFacturar  : t.netoExento;
    const dispGravado = _hayCop ? t.netoGravadoAFacturar : t.netoGravado;
    const totalIVA    = _hayCop ? t.ivaAFacturar         : t.iva;
    ...
    const pctLabel = (Math.round(pctIVAForOS(osName) * 1000) / 10) + '%';   // alícuota de la OS (data-driven)
    ...
    const filasHTML = filas.map(f => {
      ...
      return `<tr>
        ...
        <td style="font-size:12px;">${escHtml(f.prestacion)}${f.plan?' ('+f.plan+')':''}</td>
        ...
        <div class="card-body" style="background:var(--surface2);border-top:2px solid var(--border2);">
          <div style="max-width:360px;margin-left:auto;">
            ...
            <div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:15px;">
              ...
              <span style="font-weight:700;font-family:monospace;">${fmt(dispGravado)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:15px;">
            ...
            ${totalAdelanto>0?`<div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:12px;color:var(--text3);">
              <span>Coseguro a cuenta ya descontado (hereda el IVA de la prestación):</span>
              <span style="font-family:monospace;">−${fmt(totalAdelanto)}</span>
            </div>`:''}
...
// ── Atributos de FACTURACIÓN de la OS (data-driven): forma del coseguro + condición de IVA ──
// El select de IVA (exenta / 10.5 / 21) se traduce a { exentaDefault, alicuota } de la ficha.
// Con esto, dar de alta o cambiar la forma de facturar de una OS NO requiere tocar código:
// esCopagoAdelanto/getExentaForOS/pctIVAForOS leen estos campos (con fallback histórico).
function _aplicarAtributosFacturacionOS(os) {
  os.copagoModo = document.getElementById('os-copago-modo')?.value || 'complementario';
  const iva = document.getElementById('os-iva-cond')?.value || 'exenta';
  if (iva === 'exenta') { os.exentaDefault = true; delete os.alicuota; }
  else { os.exentaDefault = false; os.alicuota = parseFloat(iva); }
}
...
async function guardarOS() {
  ...
  if (editId) {
    ...
    _aplicarAtributosFacturacionOS(os);
    marcarCambios('obrasSociales');
    // Si cambió el nombre, propagarlo a TODO lo que apunta a esta OS por nombre. Sin esto,
    // las atenciones/prestaciones históricas quedaban con el nombre viejo y se orfanaban de
    // la ficha (y de su config de copago/IVA). Cada colección se marca solo si cambió algo.
    if (nombreViejo !== nombre) {
      const propagar = (arr, col, campo) => {
        let n = 0;
        (arr || []).forEach(x => { if (x[campo] === nombreViejo) { x[campo] = nombre; n++; } });
        if (n > 0) marcarCambios(col);
      };
      propagar(DB.facturas, 'facturas', 'dest');
      propagar(DB.registros, 'registros', 'os');
      propagar(DB.prestaciones, 'prestaciones', 'os');
      propagar(DB.pagosRecibidos, 'pagosRecibidos', 'os');
      propagar(DB.derivaciones, 'derivaciones', 'os');   // sin esto, una derivación quedaba huérfana de su OS
    }
    closeModal('modal-os');
    renderOS();
  ...
  _aplicarAtributosFacturacionOS(nueva);
...
function nuevaOS() {
  ...
  // Defaults para una OS nueva: coinciden con el fallback histórico de un nombre nuevo
  // (complementario + exenta), así no se cambia comportamiento salvo que se elija a mano.
  const cop = document.getElementById('os-copago-modo'); if (cop) cop.value = 'complementario';
  const ivaSel = document.getElementById('os-iva-cond'); if (ivaSel) ivaSel.value = 'exenta';
  openModal('modal-os');
}
...
function editarOS(id) {
  ...
  // Mostrar el valor EFECTIVO actual (ficha o fallback histórico), para que reguardar no
  // cambie el comportamiento sin querer. El IVA se colapsa a exenta / 10,5% / 21%.
  const cop = document.getElementById('os-copago-modo');
  if (cop) cop.value = esCopagoAdelanto(os.nombre) ? 'adelanto' : 'complementario';
  const ivaSel = document.getElementById('os-iva-cond');
  if (ivaSel) ivaSel.value = getExentaForOS(os.nombre) ? 'exenta' : (pctIVAForOS(os.nombre) >= 0.2 ? '21' : '10.5');
  openModal('modal-os');
}
...
function eliminarOS(id) {
  ...
  let msg = `¿Eliminar la obra social "${escHtml(os.nombre)}"?`;
  const avisos = [];
