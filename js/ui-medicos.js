      ...
      <div class="card" style="border-top:3px solid ${med.color};">
        <div class="card-body">
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:12px;">
            ...
            <div style="flex:1;">
              <div style="font-weight:600; font-size:14px;">${escHtml(med.nombre)}</div>
          ...
          <div style="display:flex; gap:6px;">
            ...
            <button class="btn btn-primary btn-sm" style="flex:1;" onclick="verPreliqMedico('${escJS(med.nombre)}','${mesPrefijo}')">Ver preliq.</button>
          </div>
        </div>
      </div>`;
  }).join('');
...
// ── FUENTE ÚNICA (pura/testeable) de las fichas de médico DUPLICADAS ──────────
// Agrupa por nombre normalizado (sin espacios de más, sin distinguir mayúsculas). Devuelve
// solo los grupos con más de una ficha. Cada grupo va ordenado por id ascendente (la de id
// más bajo es la "original", la que se conserva al unir). Las fichas duplicadas con el MISMO
// nombre aparecieron por el lío multi-dispositivo (dos equipos crearon la misma persona con
// ids distintos); como las atenciones se referencian por NOMBRE, al unir NO hay que re-vincular.
function _gruposMedicosDuplicados(medicos) {
  const porNombre = {};
  (medicos || []).forEach(m => {
    const k = (m && m.nombre || '').trim().toLowerCase();
    if (!k) return;
    (porNombre[k] = porNombre[k] || []).push(m);
  });
  return Object.values(porNombre)
    .filter(g => g.length > 1)
    .map(g => [...g].sort((a, b) => (Number(a.id) || 0) - (Number(b.id) || 0)));
}

// Une las fichas de médico duplicadas: conserva UNA por nombre (la original) y elimina las
// repetidas, completando los datos vacíos de la que se conserva con los de las otras. Las
// atenciones/liquidaciones/derivaciones NO se tocan (ya apuntan al mismo nombre).
function unirMedicosDuplicados() {
  const grupos = _gruposMedicosDuplicados(DB.medicos);
  if (!grupos.length) { showToast('✓ No hay médicos duplicados.'); return; }
  const totalExtra = grupos.reduce((s, g) => s + (g.length - 1), 0);
  const detalle = grupos.map(g => `• ${g[0].nombre} (${g.length} fichas)`).join('\n');
  if (!confirm(
    `UNIR MÉDICOS DUPLICADOS\n\nHay ${grupos.length} médico(s) con fichas repetidas:\n${detalle}\n\n` +
    `Se conserva UNA ficha por médico y se eliminan las ${totalExtra} repetidas. ` +
    `Las atenciones NO se tocan: siguen vinculadas al mismo nombre.\n\n¿Continuar?`
  )) return;
  let eliminadas = 0;
  grupos.forEach(g => {
    const keep = g[0];
    g.slice(1).forEach(dup => {
      // Completar SOLO los campos vacíos de la ficha que se conserva (no pisar lo que ya tiene).
      Object.keys(dup).forEach(k => {
        if (k === 'id') return;
        const vacio = keep[k] === undefined || keep[k] === null || keep[k] === '';
        if (vacio && dup[k] !== undefined && dup[k] !== null && dup[k] !== '') keep[k] = dup[k];
      });
      DB.medicos = DB.medicos.filter(m => m.id !== dup.id);
      eliminadas++;
    });
  });
  marcarCambios('medicos');
  if (typeof renderMedicosGrid === 'function') renderMedicosGrid();
  if (typeof renderConfiguracion === 'function') renderConfiguracion();
  showToast(`✓ ${eliminadas} ficha(s) duplicada(s) unificada(s)`);
}
...
// con un nombre inválido. Abre un modal con UN DESPLEGABLE por cada nombre huérfano para
// elegir con un clic a qué médico actual corresponde (sin escribir números).
let _reconectarHuerfanos = [];   // nombres huérfanos en el mismo orden que los <select> del modal
...
function reconectarMedicoHuerfano() {
  ...
  if (!nombres.length) { alert('✓ No hay nada para reconectar: todas las atenciones apuntan a un médico que existe.'); return; }
  if (!(DB.medicos || []).length) { alert('No hay médicos cargados para reasignar. Creá primero el médico correcto.'); return; }

  _reconectarHuerfanos = nombres;
  const esc = (s) => (typeof escHtml === 'function') ? escHtml(s) : s;
  const opciones = `<option value="">— dejar sin cambiar —</option>` +
    DB.medicos.map((m, mi) => `<option value="${mi}">${esc(m.nombre)}</option>`).join('');

  const body = document.getElementById('reconectar-body');
  if (body) body.innerHTML = nombres.map((nombre, i) => {
    ...
    return `<div style="border:1px solid var(--border); border-radius:8px; padding:12px; margin-bottom:10px;">
      <div style="font-weight:700; color:var(--danger); font-size:14px;">${esc(nombre)}</div>
      <div style="font-size:12px; color:var(--text3); margin:4px 0 8px;">${info.reg} atención(es) · ${info.liq} liquidación(es) · ${info.deriv} derivación(es)</div>
      <label style="font-size:12px; color:var(--text2);">Reasignar a:</label>
      <select id="reconectar-sel-${i}" style="width:100%; margin-top:4px; padding:7px; border-radius:6px;">${opciones}</select>
    </div>`;
  }).join('');

  openModal('modal-reconectar-medico');
}

// Aplica las reasignaciones elegidas en los desplegables del modal.
function confirmarReconexionMedicos() {
  const cambios = [];
  _reconectarHuerfanos.forEach((nombre, i) => {
    const sel = document.getElementById(`reconectar-sel-${i}`);
    if (!sel || sel.value === '') return;   // "dejar sin cambiar"
    const destino = (DB.medicos[parseInt(sel.value, 10)] || {}).nombre;
    if (destino) cambios.push({ nombre, destino });
  });
  if (!cambios.length) { showToast('No elegiste ningún médico para reconectar'); closeModal('modal-reconectar-medico'); return 0; }

  let totalAt = 0;
  cambios.forEach(({ nombre, destino }) => {
    let n = 0;
  ...
  marcarCambios('registros'); marcarCambios('liquidaciones'); marcarCambios('derivaciones');
  closeModal('modal-reconectar-medico');
  renderMedicosGrid && renderMedicosGrid();
  renderConfiguracion && renderConfiguracion();
  initDashboard && initDashboard();
  showToast(`✓ ${cambios.length} nombre(s) reconectado(s) · ${totalAt} atención(es)`);
...
function eliminarAtencionesHuerfanas() {
  ...
  const muestra = huerf.slice(0, 10)
    ...
    .map(r => `• ${r.fecha} · ${escHtml(r.os)} · ${fmt(fact(r))}`).join('\n');
