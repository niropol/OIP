...
function verificarPreciosAtencionesUI() {
  ...
  showToast('🔎 Verificando precios de las atenciones…');
  const res = verificarPreciosAtenciones();   // todas las OS y meses
  if (res.ok) {
    alert(`✅ Todo en orden.\n\nRevisé ${res.revisadas} atención(es). Dentro de cada mes, cada prestación tiene un único precio (no hay reprecios a medias).`);
    return;
  }
  const money = (typeof fmt === 'function') ? fmt : (n => '$' + n);
  const mesLbl = m => (typeof getMesLabel === 'function' && /^\d{4}-\d{2}$/.test(m)) ? getMesLabel(m) : (m || '—');
  const lineas = res.inconsistencias.slice(0, 20).map(d => {
    const vals = d.valores.map(v => `${v.cantidad}×${money(v.valor)}`).join(' y ');
    const hint = d.valorNomenclador != null ? `  (el nomenclador dice ${money(d.valorNomenclador)})` : '';
    return `   • ${d.os} · ${mesLbl(d.mes)} · ${d.prestacion || d.codigo || 'Consulta'}: ${vals}${hint}`;
  }).join('\n');
  const extra = res.inconsistencias.length > 20 ? `\n   …(+${res.inconsistencias.length - 20} más)` : '';
  alert(
    `⚠️ Encontré ${res.inconsistencias.length} caso(s) donde una MISMA prestación, en el MISMO mes y OS, ` +
    `tiene precios DISTINTOS (probable reprecio a medias — alguna quedó con el valor viejo):\n\n` +
    `${lineas}${extra}\n\n` +
