    ...
    return `
      <div class="card" style="margin-bottom:16px; border-top:3px solid ${med.color};">
        <div class="card-header" style="background:${med.color}11;">
          <div>
            <div style="font-size:15px; font-weight:700;">${escHtml(med.nombre)}</div>
      ...
      <div style="background:#fef9c3;border:1px solid #fde047;border-radius:8px;padding:12px 14px;margin-bottom:14px;">
        ...
        ${derivsOperadas.map(d=>`
          <div style="display:flex;justify-content:space-between;align-items:center;font-size:12px;padding:4px 0;border-bottom:1px solid #fde04780;">
            <div>
              <span style="font-weight:600;">${escHtml(d.paciente)}</span>
              <span style="color:#92400e;margin-left:6px;">${escHtml(d.cirugia)}${d.ojo?' ('+d.ojo+')':''}</span>
            </div>
            <div style="color:#78350f;font-size:11px;">${escHtml(d.os)} · ${d.fechaCx||d.fecha}</div>
          </div>`).join('')}
      </div>` : '';
...
function _renderTabCierre(medicos, mes, mesLabel, regs) {
  ...
  inner.innerHTML = medicos.map(med => {
    ...
    // ── Aviso de "foto" desactualizada: algo cambió desde que se cerró ──────────
    const drift = _driftLiquidacion(med.nombre, mes);
    const driftHTML = drift ? `
      <div style="background:#fef2f2;border:1px solid #fca5a5;border-radius:8px;padding:12px 14px;margin-bottom:14px;">
        <div style="font-size:12px;font-weight:800;color:#991b1b;margin-bottom:4px;">
          ⚠️ Los honorarios cambiaron después del cierre
        </div>
        <div style="font-size:12px;color:#7f1d1d;line-height:1.5;">
          Al cerrar (${drift.fechaCierre}) el total a depositar era <b>${fmt(drift.antes)}</b>;
          ahora sería <b>${fmt(drift.ahora)}</b> (${drift.delta>=0?'+':''}${fmt(drift.delta)}).
          ${drift.nAntes!==drift.nAhora?`Cambió la cantidad de atenciones (${drift.nAntes} → ${drift.nAhora}). `:''}
          ${drift.configCambio?`Cambió el honorario por consulta en la configuración global. `:''}
          Si el pago todavía no salió, <b>reabrí</b> para recalcular; si ya pagaste con el número viejo, dejalo así.
        </div>
      </div>` : '';
...
// ── FOTO DE LIQUIDACIÓN ───────────────────────────────────────────────────────
// La liquidación es por médico/MES (todos los consultorios), sin importar el filtro
// de pantalla. Al cerrar se guarda una "foto" de los honorarios (liq.foto); después,
// si algo cambia (se editó/agregó/borró una atención, o cambió honorarioOS), la vista
// de cierre AVISA que los números ya no coinciden con lo que se cerró.
function _regsMedicoMes(medico, mes) {
  return (DB.registros || []).filter(r => r.medico === medico && (r.fecha || '').startsWith(mes));
}

function _fotoLiquidacion(medico, mes) {
  const regs = _regsMedicoMes(medico, mes);
  const h = honorariosMedico(regs);
  const r2 = n => Math.round((n || 0) * 100) / 100;
  return {
    fecha: hoyISO(),
    aLiquidar: r2(h.aLiquidar),
    honOS: r2(h.honOS), honTr: r2(h.honTr), honSC: r2(h.honSC),
    honExtra: r2(h.honExtra), honPract: r2(h.honPract), honEf: r2(h.honEf),
    nRegistros: regs.length,
    consultas: contarConsultas(regs),
    honorarioOS: DB.config.honorarioOS,   // para explicar un drift por cambio de config global
  };
}

// Devuelve null si no hay drift (o si la liquidación es vieja y no tiene foto).
// Si los honorarios cambiaron desde el cierre, devuelve el detalle para avisar.
function _driftLiquidacion(medico, mes) {
  const liq = getLiquidacion(mes, medico);
  if (!liq || liq.estado !== 'Cerrada' || !liq.foto) return null;
  const ahora = _fotoLiquidacion(medico, mes);
  const delta = Math.round((ahora.aLiquidar - liq.foto.aLiquidar) * 100) / 100;
  const cambioRegs = ahora.nRegistros !== liq.foto.nRegistros;
  if (Math.abs(delta) < 0.01 && !cambioRegs) return null;
  return {
    antes: liq.foto.aLiquidar, ahora: ahora.aLiquidar, delta,
    nAntes: liq.foto.nRegistros, nAhora: ahora.nRegistros,
    configCambio: ahora.honorarioOS !== liq.foto.honorarioOS,
    fechaCierre: liq.foto.fecha,
  };
}
...
function cerrarMesMedico(medico, mes) {
  ...
  const liq = {
    ...
    fechaCierre: hoyISO(),
    foto: _fotoLiquidacion(medico, mes),   // foto de los honorarios al momento del cierre
