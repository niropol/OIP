...
function toggleExentaRegistro(regId, os, mes) {
  ...
  showToast(nuevo ? '✓ Atención marcada como EXENTA' : '✓ Atención marcada como GRAVADA ' + (os==='CEMEPLA'?'21%':'10.5%'));
}
...
function toggleExentaPresentacion(os, mes, codigo, desc, exentaActual) {
  ...
  showToast(nuevo
    ...
    : `✓ ${desc}: ahora GRAVADA ${os==='CEMEPLA'?'21%':'10.5%'} (${n} prestación${n!==1?'es':''})`);
}
...
function filasPresentacionOS(regs, os) {
  ...
  const pctIVA = esCemepla ? 0.21 : 0.105;
