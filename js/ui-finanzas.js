  ...
  grid.innerHTML = Object.entries(osMap).map(([os, g]) => {
    ...
    return `<div onclick="renderOSDetalle('${os.replace(/'/g,"\\'")}','${mes}')"
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;">
        <div style="font-size:14px;font-weight:800;">${escHtml(os)}</div>
        ${estadoBadge}
      </div>
...
function renderOSDetalle(os, mes) {
  ...
  if (os === 'SinCargo') {
    ...
    const filasSC = regsSC.map(r => {
      ...
      return `<tr style="border-bottom:1px solid var(--border);">
        ...
        <td style="padding:7px 10px;font-size:13px;">${escHtml(r.medico || "—")}</td>
      </tr>`;
    }).join('');
    panel.innerHTML = `
  ...
  const filasHTML = filas.map((f, idx) => {
    ...
    const filaPrincipal = `<tr style="border-bottom:1px solid var(--border);">
      ...
      ${esCemepla?`<td style="padding:7px 10px;font-size:12px;font-weight:600;">${escHtml(f.paciente)}</td><td style="padding:7px 10px;font-size:12px;">${escHtml(f.dni)}</td><td style="padding:7px 10px;font-size:12px;">${escHtml(f.empresa)}</td><td style="padding:7px 10px;font-size:12px;">${escHtml(f.autorizacion||'—')}</td>`:''}
      <td style="padding:7px 10px;font-size:12px;">${escHtml(f.prestacion)}${f.plan?' ('+escHtml(f.plan)+')':''}</td>
  ...
  panel.innerHTML = `
    <div class="card" style="border-top:3px solid var(--accent2);">
      <div class="card-header">
        <div>
          <div style="font-size:15px;font-weight:700;">${escHtml(os)} — ${mesLabel}</div>
...
function copiarDetalleOSMail(os, mes) {
  ...
  const filasHTML = filas.map(f=>{
    ...
    return `<tr>
      ...
      ${esCemepla?`<td style="${tdS}">${escHtml(f.paciente)}</td><td style="${tdS}">${escHtml(f.dni)}</td><td style="${tdS}">${escHtml(f.empresa)}</td><td style="${tdS}">${escHtml(f.autorizacion||'—')}</td>`:''}
      <td style="${tdS}">${escHtml(f.prestacion)}${f.plan?' ('+escHtml(f.plan)+')':''}</td>
...
function renderOSTbody() {
  ...
  tbody.innerHTML = DB.obrasSociales.map(os => {
    ...
    return `<tr>
      <td style="font-weight:600;">${escHtml(os.nombre)}</td>
      <td>${os.pago}</td>
  ...
  tbody.innerHTML = data.map(f => {
    ...
    return `
      <tr class="${f.estado==='Vencida'?'row-alert':dias<=10&&f.estado!=='Vencida'?'row-warn':''}">
        <td style="font-family:monospace;font-size:12px;font-weight:700;">${escHtml(f.num)}</td>
        <td style="font-weight:600;">${escHtml(f.dest)}</td>
        <td>${f.mes}</td>
        ...
        <td style="max-width:140px;font-size:11px;color:var(--text2);">${escHtml(f.obs||'—')}</td>
        <td>
...
function abrirConfirmarCobro(facturaId) {
  ...
  document.getElementById('cobro-factura-info').innerHTML = `
    <div style="display:flex; gap:20px; flex-wrap:wrap;">
      <div><span style="font-size:11px; color:var(--text3);">N° Factura</span><div style="font-weight:700; font-family:monospace;">${escHtml(f.num)}</div></div>
      <div><span style="font-size:11px; color:var(--text3);">Obra Social</span><div style="font-weight:700;">${escHtml(f.dest)}</div></div>
...
function confirmarCobro() {
  ...
  DB.movimientos.unshift({
    id: DB.nextId++,
    fecha,
    desc: `Cobro ${escHtml(f.num)} — ${escHtml(f.dest)} (${f.mes})`,
    consultorio: 'General',
    tipo: 'Ingreso',
    monto,
    saldo: 0,
  ...
  showToast(Math.abs(diferencia) < 0.005
    ? `✓ Cobro de ${escHtml(f.num)} confirmado — ${fmt(monto)}`
    : `✓ Cobro de ${escHtml(f.num)} — ${fmt(monto)} · ⚠️ ${diferencia < 0 ? 'faltan' : 'sobran'} ${fmt(Math.abs(diferencia))} vs. lo facturado`);
  renderCobranzas();
  renderOS();
}
...
function eliminarFactura() {
  ...
  let msg = `¿Eliminar la factura ${escHtml(f.num)} (${escHtml(f.dest)} · ${f.mes})?`;
...
function abrirNota(facturaId) {
  ...
  document.getElementById('nota-factura-info').innerHTML = `<strong>${escHtml(f.num)}</strong> · ${escHtml(f.dest)} · ${f.mes} · ${fmt(f.monto)}`;
...
function marcarEnviada(facturaId) {
  ...
  f.estado = 'Pendiente';
  marcarCambios('facturas');
  showToast(`✓ ${escHtml(f.num)} marcada como enviada a ${escHtml(f.dest)}`);
  renderCobranzas();
}
...
function renderFinanzas() {
  ...
  if (g('fin-caja-sub'))    g('fin-caja-sub').textContent    = saldosPorConsultorio.map(c=>`${escHtml(c.nombre)}: ${fmt(c.saldo)}`).join(' · ');
  ...
  if (resumen) {
    ...
    resumen.innerHTML = entries.map(([os, d]) => {
      ...
      return `<div class="stat-card" style="border-left:3px solid ${hayVencida?'var(--danger)':'var(--warn)'};">
        <div class="stat-label">${escHtml(os)}</div>
      ...
      : `<div style="display:flex; gap:0; overflow-x:auto; padding-bottom:4px;">` +
        prox.map(f => {
          ...
          return `<div style="flex:0 0 auto; min-width:150px; border-left:3px solid ${color}; padding:10px 14px; margin-right:12px; background:var(--surface2); border-radius:0 6px 6px 0;">
            <div style="font-size:10px; color:var(--text3); text-transform:uppercase; font-weight:600; margin-bottom:3px;">${escHtml(f.dest)}</div>
            <div style="font-family:monospace; font-size:11px; color:var(--text2);">${escHtml(f.num)}</div>
  ...
  tbody.innerHTML = pending.map(f => {
    ...
    return `
      <tr class="${f.estado==='Vencida'?'row-alert':dias<=10?'row-warn':''}">
        <td style="font-family:monospace; font-size:12px; font-weight:700; white-space:nowrap;">${escHtml(f.num)}</td>
        <td style="font-weight:600;">${escHtml(f.dest)}</td>
        <td>${f.mes}</td>
        <td style="text-align:center;">${f.prestaciones}</td>
        <td class="finance-num" style="font-weight:700;">${fmt(f.monto)}</td>
            ...
            <table style="width:100%; border-collapse:collapse; font-size:12px;">
              ...
              <tbody>
                ...
                <tr style="border-top:1px solid var(--border);">
                  <td style="padding:4px 8px; font-family:monospace;">${escHtml(f.num)}</td>
                  <td style="padding:4px 8px; font-weight:600;">${escHtml(f.dest)}</td>
          ...
          <div class="card-body">
            ...
            ${huerfanos.map(m => `
              <div style="display:flex; align-items:center; gap:10px; padding:8px; border-bottom:1px solid var(--border); flex-wrap:wrap;">
                <span style="flex:1; font-size:13px;">${m.fecha} · ${m.tipo} · ${escHtml(m.concepto)} · <b>${fmt(m.monto)}</b>${m.consultorio?` · <span style="color:var(--text3);">(actual: ${escHtml(m.consultorio)})</span>`:''}</span>
...
function renderTablaCajaChica(consultorio, tbodyId) {
  ...
  tbody.innerHTML = conSaldo.length === 0
    ...
    : conSaldo.map(m => `
      <tr>
        <td>${m.fecha}</td>
        <td style="font-size:13px;">${escHtml(m.concepto)}</td>
        <td><span class="pill ${m.tipo==='Ingreso'?'pill-paid':'pill-overdue'}">${m.tipo}</span></td>
        <td><span class="pill" style="${origenColors[m.origen]||''}">${escHtml(m.origen)}</span></td>
...
function editarCajaChica(id) {
  ...
  if (m.origen && m.origen !== 'Manual') {
    if (!confirm(`Este movimiento se generó automáticamente (${escHtml(m.origen)}) a partir de una atención.\n\nEditarlo cambia solo la caja, no la atención que lo originó. ¿Continuar?`)) return;
  }
  _editCajaId = id;
...
function eliminarCajaChica(id) {
  ...
  const aviso = (m.origen && m.origen !== 'Manual')
    ? `\n\n⚠️ Este movimiento se generó automáticamente (${escHtml(m.origen)}). Eliminarlo no modifica la atención que lo originó.`
    : '';
  if (!confirm(`¿Eliminar este movimiento?\n\n${m.tipo} · ${escHtml(m.concepto)} · ${fmt(m.monto)}${aviso}`)) return;
