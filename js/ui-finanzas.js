...
// IVA (corrige un error de carga). Afecta a todos los registros de esa OS+mes que
// hoy coinciden con esa fila: misma prestación/plan/valor unitario y mismo estado
// de exención actual. CEMEPLA no se toca (siempre gravada al 21%).
function toggleExentaPresentacion(os, mes, codigo, desc, exentaActual) {
  ...
  // Si ya hay factura hecha para este mes, cambiar la exención cambia el total que
  // debería pagar la OS: avisar para que se corrija/regenere la factura después.
  ...
  if (fact) {
    ...
    if (!confirm(`Ya existe la factura ${fact.num} (${fact.estado}) para ${os} · ${mesLabel}.\n\n¿Cambiar igual la exención de "${desc}"?${extra}`)) return;
  }
  ...
  // Volver a dibujar el panel (recalculado) sin que el toggle-cerrar lo oculte, y
  // refrescar cards/stats porque cambian los totales de neto exento/gravado e IVA.
  const panel = document.getElementById('os-detalle-panel');
  if (panel) panel.dataset.os = '';
...
function filasPresentacionOS(regs, os) {
  ...
  regs.forEach(r => {
    ...
    if (!grupos[key]) grupos[key] = {
      ...
      exenta, os, cant: 0, neto: 0, copagoAdel: 0, _valores: new Set(),
    };
    const g = grupos[key];
    g.cant += r.cantidad;
  ...
  return Object.values(grupos).map(g => ({
    ...
    cant: g.cant, neto: g.neto, copagoAdel: g.copagoAdel,
    ...
    return `<tr style="border-bottom:1px solid var(--border);">
      ...
      <td style="padding:7px 10px;text-align:center;">
        ${esCemepla
          ...
          : `<span class="pill" role="button" title="Clic para cambiar entre exenta y gravada"
