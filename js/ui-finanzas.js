...
// Banner persistente: meses ya facturados cuyo total actual (tras un reprecio o edición)
// no coincide con lo facturado. Muestra la diferencia y ofrece registrar la complementaria.
function renderDiferenciasFacturar() {
  const cont = document.getElementById('os-diferencias-banner');
  if (!cont) return;
  const difs = (typeof _diferenciasAFacturar === 'function') ? _diferenciasAFacturar() : [];
  const pendientes = difs.filter(d => d.diferencia > 0);   // solo "falta facturar" (positivas)
  if (pendientes.length === 0) { cont.innerHTML = ''; return; }
  const filas = pendientes.map(d => `
    <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; padding:8px 0; border-top:1px solid #fde68a; flex-wrap:wrap;">
      <div style="font-size:13px; color:#78350f;">
        <b>${d.os}</b> · ${d.mesLabel} — facturado ${fmt(d.yaFacturado)}, ajustado ${fmt(d.totalAjustado)}
        → <b style="color:#b45309;">falta facturar ${fmt(d.diferencia)}</b>
      </div>
      <button class="btn btn-secondary btn-sm" onclick="registrarFacturaComplementaria('${escJS(d.os)}','${d.mes}')">
        🧾 Registrar complementaria
      </button>
    </div>`).join('');
  cont.innerHTML = `
    <div style="background:#fffbeb; border:1px solid #fcd34d; border-radius:10px; padding:14px 18px; margin-bottom:18px;">
      <div style="font-size:13px; font-weight:800; color:#92400e; margin-bottom:2px;">
        ⚠️ ${pendientes.length} mes(es) facturado(s) con diferencia pendiente
      </div>
      <div style="font-size:11px; color:#a16207; margin-bottom:6px;">
        El total de estas atenciones cambió después de facturar. Emití una factura complementaria por la diferencia (la factura original no se toca).
      </div>
      ${filas}
    </div>`;
}

// Al cambiar el mes en el desplegable de OS/Pagos → Resumen: hay que volver a
// dibujar las CARDS del mes nuevo (antes el onchange solo llamaba a
// renderOSDetalle() sin argumentos, así que las tarjetas quedaban en el mes
// anterior y parecía que "no cambiaba"). Además se cierra el panel de detalle,
// que podía haber quedado abierto para una OS del mes viejo.
function onOSMesChange() {
  cerrarPanelOSDetalle();
  renderOSCards();
  renderOSStats();
}

// Cierra el panel de detalle de OS (la "presentación" abierta). Se usa al cambiar
// de mes y después de editar/eliminar una factura, para no dejar el panel mostrando
// datos que ya cambiaron.
function cerrarPanelOSDetalle() {
  const panel = document.getElementById('os-detalle-panel');
  if (panel) { panel.style.display = 'none'; panel.dataset.os = ''; }
}

// Escapa un texto para usarlo DENTRO de un string JS de un onclick inline (atributo
// HTML con comillas dobles). Escapa \, ' y " (esta última como &quot; para no cerrar
// el atributo; el navegador la decodifica a " antes de parsear el JS).
function escJS(s) {
  return String(s == null ? '' : s).replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/"/g, '&quot;');
}

// Muestra/oculta el sub-detalle de atenciones de una fila agrupada de la presentación.
function toggleFilaAtenciones(grpId, btn) {
  const row = document.getElementById(grpId);
  if (!row) return;
...
// Aviso compartido: si ya hay factura hecha para ese OS/mes, cambiar la exención cambia
// el total que debería pagar la OS. Devuelve true si se puede seguir (o no hay factura).
function _confirmarCambioExencion(os, mes, queCambia) {
  const mesLabel = getMesLabel(mes);
  const fact = DB.facturas.find(f => f.dest===os && f.mes===mesLabel && ['Preliquidada','Pendiente','Pagada'].includes(f.estado));
  if (!fact) return true;
  const extra = fact.estado==='Pagada'
    ? `\n\n⚠️ Además ya está COBRADA (${fmt(fact.monto)}): si el total cambia, vas a tener que corregir la factura y el ingreso en Caja a mano.`
    : `\n\nRecordá corregir/regenerar la factura ${fact.num} para que el monto cuadre con el nuevo IVA.`;
  return confirm(`Ya existe la factura ${fact.num} (${fact.estado}) para ${os} · ${mesLabel}.\n\n¿Cambiar igual ${queCambia}?${extra}`);
}

// Re-dibuja el panel de detalle de la OS (recalculado) y las cards/stats. Se usa tras
// corregir una exención. La exención (IVA) NO afecta el honorario del médico, así que
// esto se puede hacer aunque la liquidación del médico esté cerrada.
function _refrescarTrasCambioExencion(os, mes) {
  marcarCambios('registros');
  const panel = document.getElementById('os-detalle-panel');
  if (panel) panel.dataset.os = '';   // evitar el toggle-cerrar de renderOSDetalle
  renderOSDetalle(os, mes);
  renderOSCards();
  renderOSStats();
}

// Corrige la exención de UNA atención puntual (por id), desde el sub-detalle de la
// presentación. No toca las demás del código. Sirve para el caso típico: una consulta
// que se cargó gravada cuando debía ir exenta, con las otras del mismo código bien.
function toggleExentaRegistro(regId, os, mes) {
  if (os === 'CEMEPLA') { showToast('CEMEPLA siempre va gravada al 21%.'); return; }
  const r = DB.registros.find(x => x.id === regId);
  if (!r) { showToast('No se encontró la atención.'); return; }
  const nuevo = !exentaReg(r, os);
  if (!_confirmarCambioExencion(os, mes, 'esta atención')) return;
  r.exenta = nuevo;
  _refrescarTrasCambioExencion(os, mes);
  showToast(nuevo ? '✓ Atención marcada como EXENTA' : '✓ Atención marcada como GRAVADA ' + pctIVALabelOS(os));
}

// Cambiar, desde la presentación a la OS, si una prestación va EXENTA o GRAVADA de
// IVA. Afecta a TODAS las atenciones de ese código (mismo estado de exención actual).
// Para corregir una sola, usar toggleExentaRegistro desde el sub-detalle (▸).
// CEMEPLA no se toca (siempre gravada al 21%).
function toggleExentaPresentacion(os, mes, codigo, desc, exentaActual) {
  if (os === 'CEMEPLA') { showToast('CEMEPLA siempre va gravada al 21%.'); return; }
  const nuevo = !exentaActual;
  if (!_confirmarCambioExencion(os, mes, `la exención de "${desc}" (todo el código)`)) return;

  // Buscar el código de un registro igual que en el agrupado del panel.
  const codigoDe = (osNom, d) => {
    const p = (DB.prestaciones || []).find(x => x.os === osNom && x.desc === d);
    return p ? (p.codigo || '—') : '—';
  };
  const conCodigo = codigo && codigo !== '—';

  let n = 0;
  DB.registros.forEach(r => {
    if (r.os !== os || !r.fecha || !r.fecha.startsWith(mes)) return;
    const cod = codigoDe(os, r.prestacion || 'Consulta');
    // Con código: todas las del mismo código. Sin código: solo las de esa descripción.
    if (conCodigo ? cod !== codigo : ((r.prestacion || 'Consulta') !== desc || (cod && cod !== '—'))) return;
    if (exentaReg(r, os) !== exentaActual) return;   // solo las que hoy están en el estado de esta fila
    r.exenta = nuevo;
    n++;
  });
  if (n === 0) { showToast('No se encontraron prestaciones para cambiar.'); return; }
  _refrescarTrasCambioExencion(os, mes);
  showToast(nuevo
    ? `✓ ${desc}: ahora EXENTA (${n} prestación${n!==1?'es':''})`
    : `✓ ${desc}: ahora GRAVADA ${pctIVALabelOS(os)} (${n} prestación${n!==1?'es':''})`);
}

// ── Cards de OS con atenciones del mes seleccionado ──────────────────────────
function renderOSCards() {
  const mes = document.getElementById('os-fact-mes')?.value || hoyISO().slice(0,7);
  const mesLabel = getMesLabel(mes);
  ...
  grid.innerHTML = Object.entries(osMap).map(([os, g]) => {
    const total   = g.total;
    const pctIVA  = Math.round(pctIVAForOS(os) * 1000) / 10;   // alícuota de la OS (data-driven)
...
// ── FUENTE ÚNICA de las filas de una presentación a la OS ─────────────────────
// Devuelve las filas que muestran el detalle en pantalla (renderOSDetalle) y el
// resumen que se le manda a la OS (generarPreliqOS), para que las dos vistas sean
// idénticas. Agrupa POR CÓDIGO: junta todas las prestaciones del mismo código en
// una sola fila (más claro y limpio para la OS), sin importar la fecha ni pequeñas
// diferencias de texto. Solo mantiene separado exento/gravado (van en buckets de
// IVA distintos, no se pueden sumar). CEMEPLA NO se agrupa: una fila por paciente.
// Prestaciones sin código en el nomenclador (—) se agrupan por descripción para no
// mezclar cosas distintas.
function filasPresentacionOS(regs, os) {
  const esCemepla = os === 'CEMEPLA';
  const pctIVA = pctIVAForOS(os);   // alícuota de la OS (data-driven) — MISMO criterio que totalesOS()
  const codigoDe = (osNom, desc) => {
    const p = (DB.prestaciones || []).find(x => x.os === osNom && x.desc === desc);
    return p ? (p.codigo || '—') : '—';
  };
  // Descripción "oficial" del código (la del nomenclador), para que el resumen use
  // siempre el mismo nombre aunque las atenciones se hayan cargado con textos distintos.
  const descPresentacion = (osNom, codigo, fallback) => {
    if (!codigo || codigo === '—') return fallback;
    const p = (DB.prestaciones || []).find(x => x.os === osNom && x.codigo === codigo);
    return p && p.desc ? p.desc : fallback;
  };
  ...
  if (esCemepla) {
    return regs.map(r => {
      ...
      return {
        ...
        exenta: false,                        // CEMEPLA siempre gravada
        os, valorUnit: r.valorUnit, cant: r.cantidad, neto, iva: neto * pctIVA,
        copagoAdel: copagoAdelantoReg(r),
        codigo: codigoDe(os, r.prestacion || 'Consulta'), valorMixto: false,
  ...
  regs.forEach(r => {
    ...
    const key = (codigo && codigo !== '—') ? `${codigo}||${exenta}` : `—||${desc}||${exenta}`;
    if (!grupos[key]) grupos[key] = {
      codigo, prestacion: descPresentacion(os, codigo, desc), plan: r.plan || '',
      exenta, os, cant: 0, neto: 0, copagoAdel: 0, _valores: new Set(), miembros: [],
    };
    const g = grupos[key];
    g.cant += r.cantidad;
    g.neto += (r.cantidad || 0) * (r.valorUnit || 0);
    g.copagoAdel += copagoAdelantoReg(r);
    g._valores.add(r.valorUnit || 0);
    g.miembros.push(r);   // registros individuales del grupo (para corregir uno puntual)
  });
  // Ordenar: exentas primero, luego gravadas; dentro, por código.
  return Object.values(grupos).map(g => ({
    codigo: g.codigo, prestacion: g.prestacion, plan: g.plan, exenta: g.exenta, os: g.os,
    cant: g.cant, neto: g.neto, copagoAdel: g.copagoAdel, miembros: g.miembros,
    iva: g.exenta ? 0 : g.neto * pctIVA,            // mismo criterio que totalesOS()
    // valor unitario a mostrar: el común si no cambió en el mes; si hubo distintos
    // precios para el mismo código, el promedio ponderado (neto/cant).
    valorUnit: g._valores.size === 1 ? [...g._valores][0] : (g.cant ? g.neto / g.cant : 0),
    valorMixto: g._valores.size > 1,
  })).sort((a,b) =>
...
function renderOSDetalle(os, mes) {
  ...
  // Filas de la presentación (agrupadas por código) — fuente única compartida con
  // el resumen que se manda a la OS (generarPreliqOS).
  const filas = filasPresentacionOS(regs, os);
  const pctLabel = (Math.round(pctIVAForOS(os) * 1000) / 10) + '%';   // alícuota de la OS (data-driven)

  const colspanDetalle = 7 + (manejaCopago ? 1 : 0);   // columnas de la tabla (no-CEMEPLA)
  const filasHTML = filas.map((f, idx) => {
    ...
    // Las filas agrupadas (no-CEMEPLA) con más de una atención se pueden expandir para
    // corregir la exención de UNA atención puntual (sin afectar a las otras del código).
    const miembros = f.miembros || [];
    const expandible = !esCemepla && miembros.length > 0;
    const grpId = `os-grp-${idx}`;
    const btnExpand = expandible
      ? `<span onclick="toggleFilaAtenciones('${grpId}', this)" title="Ver atenciones y corregir una puntual" style="cursor:pointer;color:var(--accent2);font-weight:700;margin-right:6px;user-select:none;">▸</span>`
      : '';

    const filaPrincipal = `<tr style="border-bottom:1px solid var(--border);">
      <td style="padding:7px 10px;font-family:monospace;font-size:12px;">${btnExpand}${fechaCell}</td>
      ...
      <td style="padding:7px 10px;text-align:right;font-family:monospace;">${fmt(f.valorUnit)}${f.valorMixto?'<span title="Hubo distintos precios para este código en el mes; se muestra el promedio" style="color:var(--text3);cursor:help;"> ~</span>':''}</td>
      <td style="padding:7px 10px;text-align:center;">
        ${esCemepla
          ? `<span class="pill" style="background:var(--warn-light);color:var(--warn);font-size:10px;">⚡ ${pctLabel}</span>`
          : `<span class="pill" role="button" title="Cambia TODO el código a la vez. Para corregir una sola atención, expandí con ▸"
                onclick="toggleExentaPresentacion('${escJS(os)}','${mes}','${escJS(f.codigo||'—')}','${escJS(f.prestacion||'Consulta')}',${f.exenta})"
                style="cursor:pointer;${f.exenta?'background:var(--success-light);color:var(--success);':'background:var(--warn-light);color:var(--warn);'}font-size:10px;">
    ...
    if (!expandible) return filaPrincipal;

    // Sub-fila oculta: una línea por atención individual, con su propio toggle exenta/gravada.
    const lineas = [...miembros].sort((a,b) => (a.fecha||'').localeCompare(b.fecha||'')).map(r => {
      const [yy,mm,dd] = (r.fecha||'--').split('-');
      const quien = r.paciente || (r.medico ? soloApellido(r.medico) : '') || '—';
      const netoR = (r.cantidad||0) * (r.valorUnit||0);
      const exR = exentaReg(r, os);
      return `<div style="display:flex;align-items:center;gap:12px;padding:6px 14px 6px 30px;border-top:1px solid var(--border);font-size:12px;">
        <span style="font-family:monospace;color:var(--text3);width:60px;">${dd&&mm?dd+'/'+mm:'—'}</span>
        <span style="flex:1;">${quien}${r.medico&&r.paciente?' · '+soloApellido(r.medico):''}${(r.cantidad||1)>1?' · x'+r.cantidad:''}</span>
        <span style="font-family:monospace;color:var(--text3);">${fmt(netoR)}</span>
        <span class="pill" role="button" title="Corregir SOLO esta atención"
          onclick="toggleExentaRegistro(${r.id},'${escJS(os)}','${mes}')"
          style="cursor:pointer;${exR?'background:var(--success-light);color:var(--success);':'background:var(--warn-light);color:var(--warn);'}font-size:10px;">
          ${exR?'✓ Exento':'⚡ '+pctLabel} ✎
        </span>
      </div>`;
    }).join('');
    const subFila = `<tr id="${grpId}" style="display:none;"><td colspan="${colspanDetalle}" style="padding:0;background:var(--surface2);">
      <div style="padding:4px 0;">
        <div style="font-size:11px;color:var(--text3);padding:4px 14px 2px 30px;">Atenciones de este código — tocá el estado para corregir una sola:</div>
        ${lineas}
      </div>
    </td></tr>`;
    return filaPrincipal + subFila;
  }).join('');

  const { netoExento, netoGravado, iva: ivaGravado, copagoAdelanto: totalCopagoAdel, total,
          netoExentoAFacturar, netoGravadoAFacturar, ivaAFacturar } = totOS;
  // Desglose a MOSTRAR y a FACTURAR: si hay copago a cuenta, el coseguro hereda el IVA de la
  // prestación y se descuenta de su bucket (neto de copago). Si no, el bruto de siempre.
  const hayCopagoAdel = manejaCopago && totalCopagoAdel > 0;
  const dispExento  = hayCopagoAdel ? netoExentoAFacturar  : netoExento;
  const dispGravado = hayCopagoAdel ? netoGravadoAFacturar : netoGravado;
  const dispIVA     = hayCopagoAdel ? ivaAFacturar         : ivaGravado;
  const dispTotal   = total - totalCopagoAdel;   // a facturar a la OS (igual que antes)
  ...
  // Incluye las atenciones con paciente, autorización O comentario (el comentario de una
  // práctica/cirugía es una aclaración para la OS). Se agrega la columna "Observación".
  const regsConDato = !esCemepla ? regs.filter(r => r.paciente || r.autorizacion || r.comentario).sort((a,b) => a.fecha.localeCompare(b.fecha)) : [];
  const hayComent = regsConDato.some(r => r.comentario);
  const detalleAutorizacionHTML = regsConDato.length ? `
      <div class="card-body" style="border-top:2px solid var(--border2);">
        <div style="font-size:13px;font-weight:700;margin-bottom:8px;">🔖 Pacientes / N° de autorización${hayComent ? ' / observaciones' : ''}</div>
        <table style="width:100%;border-collapse:collapse;font-size:12px;">
          <thead><tr style="background:var(--surface2);">
            ...
            ${hayComent ? '<th style="padding:6px 10px;text-align:left;">Observación</th>' : ''}
          </tr></thead>
          <tbody>${regsConDato.map(r => {
            const [y,m,d] = r.fecha.split('-');
            return `<tr style="border-bottom:1px solid var(--border);">
              <td style="padding:6px 10px;font-family:monospace;">${d}/${m}</td>
              <td style="padding:6px 10px;font-weight:600;">${_esc(r.paciente) || '—'}</td>
              <td style="padding:6px 10px;">${_esc(r.prestacion) || 'Consulta'}</td>
              <td style="padding:6px 10px;">${_esc(r.autorizacion) || '—'}</td>
              ${hayComent ? `<td style="padding:6px 10px;color:var(--text2);">${_esc(r.comentario) || '—'}</td>` : ''}
            </tr>`;
          }).join('')}</tbody>
        </table>
      </div>` : '';
      ...
      <div class="card-body" style="background:var(--surface2);border-top:2px solid var(--border2);">
        ...
        <div style="display:flex;gap:24px;flex-wrap:wrap;align-items:flex-end;justify-content:space-between;">
          <div style="min-width:280px;">
            ${dispExento>0 ? `<div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:15px;">
              <span style="color:var(--text2);">Neto exento:</span>
              <span style="font-weight:700;font-family:monospace;">${fmt(dispExento)}</span>
            </div>` : ''}
            ${dispGravado>0 ? `<div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:15px;">
              <span style="color:var(--text2);">Neto gravado:</span>
              <span style="font-family:monospace;font-weight:700;">${fmt(dispGravado)}</span>
            </div>
            <div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:15px;">
              ...
              <span style="font-family:monospace;color:var(--warn);font-weight:700;">${fmt(dispIVA)}</span>
            </div>` : ''}
            ${hayCopagoAdel ? `<div style="display:flex;justify-content:space-between;gap:20px;margin-bottom:7px;font-size:12px;color:var(--text3);">
              <span>Coseguro a cuenta ya descontado (hereda el IVA de la prestación):</span>
              <span style="font-family:monospace;">−${fmt(totalCopagoAdel)}</span>
            </div>` : ''}
            <div style="display:flex;justify-content:space-between;gap:20px;font-size:22px;font-weight:800;border-top:2px solid var(--border2);padding-top:10px;margin-top:6px;">
              <span>${hayCopagoAdel ? 'A facturar a la OS:' : 'TOTAL:'}</span>
              <span style="font-family:monospace;color:var(--accent2);">${fmt(dispTotal)}</span>
            </div>
          </div>
          <div>
            ${factExist
              ? `<div style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
                  ...
                  <button class="btn btn-secondary" style="font-size:13px;" onclick="editarFactura(${factExist.id})">
                    ✏️ Editar / corregir presentación
                  </button>
                </div>`
              : `<button class="btn btn-primary" style="font-size:14px;padding:10px 24px;" onclick="abrirConfirmarFactura('${os.replace(/'/g,"\\'")}','${mes}','${mesLabel}',${dispTotal},${dispExento},${dispGravado},${dispIVA},${filas.reduce((s,f)=>s+f.cant,0)},${totalCopagoAdel})">
                  🧾 Hacer factura
                </button>`
            }
          </div>
...
function copiarDetalleOSMail(os, mes) {
  ...
  // Filas de la presentación (agrupadas por código) — misma fuente única que el
  // detalle en pantalla, para que el mail a la OS coincida exactamente con lo que se ve.
  const filas = filasPresentacionOS(regs, os);

  const { netoExento, netoGravado, iva: ivaGravado, copagoAdelanto: totalCopagoAdel, total,
          netoExentoAFacturar, netoGravadoAFacturar, ivaAFacturar } = totOS;
  // Desglose a mostrar: neto de copago si hay coseguro a cuenta (hereda el IVA de la prestación).
  const _hayCop = manejaCopago && totalCopagoAdel > 0;
  const dispExento  = _hayCop ? netoExentoAFacturar  : netoExento;
  const dispGravado = _hayCop ? netoGravadoAFacturar : netoGravado;
  const dispIVA     = _hayCop ? ivaAFacturar         : ivaGravado;
  const resEx={}, resGr={};
  ...
  const filasHTML = filas.map(f=>{
    ...
    return `<tr>
      ...
      <td style="${tdS}text-align:right;">${fmt(f.valorUnit)}${f.valorMixto?' ~':''}</td>
  ...
  // Detalle paciente + N° de autorización + observación aparte (sobre todo cirugías/prácticas)
  // — igual criterio que renderOSDetalle: solo para OS que agrupan por código.
  const _escM = (s) => (typeof escHtml === 'function' ? escHtml(String(s ?? '')) : String(s ?? ''));
  const regsConDato = !esCemepla ? regs.filter(r => r.paciente || r.autorizacion || r.comentario).sort((a,b) => a.fecha.localeCompare(b.fecha)) : [];
  const hayComentM = regsConDato.some(r => r.comentario);
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:760px;color:#1f2937;">
    ...
    <table style="border-collapse:collapse;margin-top:12px;font-size:15px;">
      ${dispExento>0?`<tr><td style="padding:4px 16px 4px 0;">Neto exento:</td><td style="padding:4px 0;text-align:right;font-weight:700;">${fmt(dispExento)}</td></tr>`:''}
      ${dispGravado>0?`<tr><td style="padding:4px 16px 4px 0;">Neto gravado:</td><td style="padding:4px 0;text-align:right;font-weight:700;">${fmt(dispGravado)}</td></tr>
      <tr><td style="padding:4px 16px 4px 0;">IVA ${pctLabel}:</td><td style="padding:4px 0;text-align:right;font-weight:700;color:#b45309;">${fmt(dispIVA)}</td></tr>`:''}
      ${_hayCop?`<tr><td style="padding:4px 16px 4px 0;font-size:12px;color:#6b7280;">Coseguro a cuenta ya descontado (hereda el IVA de la prestación):</td><td style="padding:4px 0;text-align:right;font-size:12px;color:#6b7280;">−${fmt(totalCopagoAdel)}</td></tr>`:''}
      <tr><td style="padding:8px 16px 4px 0;font-size:19px;font-weight:800;border-top:2px solid #1e3a5f;">${_hayCop?'A facturar a la OS:':'TOTAL:'}</td><td style="padding:8px 0 4px 0;text-align:right;font-size:19px;font-weight:800;color:#1d4ed8;border-top:2px solid #1e3a5f;">${fmt(total-totalCopagoAdel)}</td></tr>
    </table>
    ${regsConDato.length ? `
    <div style="font-size:14px;font-weight:700;margin-top:18px;margin-bottom:6px;">Pacientes / N° de autorización${hayComentM ? ' / observaciones' : ''}</div>
    <table style="border-collapse:collapse;width:100%;">
      <thead><tr>
        ...
        ${hayComentM ? `<th style="${thS}text-align:left;">Observación</th>` : ''}
      </tr></thead>
      <tbody>${regsConDato.map(r => {
        const [y,m,d] = r.fecha.split('-');
        return `<tr>
          <td style="${tdS}">${d}/${m}</td>
          <td style="${tdS}">${_escM(r.paciente) || '—'}</td>
          <td style="${tdS}">${_escM(r.prestacion) || 'Consulta'}</td>
          <td style="${tdS}">${_escM(r.autorizacion) || '—'}</td>
          ${hayComentM ? `<td style="${tdS}">${_escM(r.comentario) || '—'}</td>` : ''}
        </tr>`;
      }).join('')}</tbody>
    </table>` : ''}
  </div>`;
...
function abrirConfirmarFactura(os, mes, mesLabel, total, netoExento, netoGravado, ivaGravado, cant, copagoAdel) {
  ...
  document.getElementById('cf-resumen').innerHTML = `
    <div style="font-size:13px;line-height:2;">
      ...
      ${copagoAdel>0?`<div style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:12px;color:var(--text3);">
        <span>Coseguro a cuenta ya descontado (hereda el IVA de la prestación):</span>
        <span style="font-family:monospace;">−${fmt(copagoAdel)}</span>
      </div>`:''}
  ...
  // Monto final editable: arranca en el total calculado.
  const _mf = document.getElementById('cf-monto-final');
  if (_mf) _mf.value = round2(total);
  _actualizarMontoFacturaCalc();

  openModal('modal-confirmar-factura');
}

// Muestra el total calculado como referencia y, si el usuario ajusta el monto final,
// cuánto se aparta de lo calculado. No bloquea: el usuario factura el número real.
function _actualizarMontoFacturaCalc() {
  const modal = document.getElementById('modal-confirmar-factura');
  const el = document.getElementById('cf-monto-calc');
  if (!modal || !el) return;
  const calc = parseFloat(modal.dataset.total) || 0;
  const val = parseFloat(document.getElementById('cf-monto-final')?.value);
  if (isNaN(val)) { el.textContent = `Calculado por el sistema: ${fmt(calc)}`; el.style.color = 'var(--text3)'; return; }
  const dif = Math.round((val - calc) * 100) / 100;
  if (Math.abs(dif) < 0.005) { el.textContent = `✓ Coincide con lo calculado (${fmt(calc)})`; el.style.color = 'var(--text3)'; }
  else { el.textContent = `Calculado: ${fmt(calc)} · Ajuste: ${dif > 0 ? '+' : ''}${fmt(dif)}`; el.style.color = 'var(--warn)'; }
}
...
function confirmarFacturaOS() {
  ...
  // Monto FINAL editable (el usuario puede ajustar el número real que facturó). Si lo dejó
  // vacío, se usa el calculado. netoExento/gravado/IVA quedan como el detalle calculado.
  const _montoFinal = parseFloat(document.getElementById('cf-monto-final')?.value);
  const _montoCalc  = parseFloat(modal.dataset.total);
  const total     = isNaN(_montoFinal) ? _montoCalc : _montoFinal;
  if (isNaN(total) || total <= 0) { showToast('⚠️ El monto a facturar debe ser mayor a 0.'); return; }
  // Marca: el monto se ajustó a mano y difiere del calculado. Sirve para que el diagnóstico
  // NO lo reporte como "el monto no cierra con el desglose": la diferencia es deliberada.
  const montoAjustado = !isNaN(_montoCalc) && Math.abs(total - _montoCalc) > 0.005;
  ...
  marcarCambios('facturas'); DB.facturas.push({
    ...
    pctIVA,
    monto:        total,
    montoAjustado,          // true = el monto se puso a mano distinto del calculado
    montoCalculado: isNaN(_montoCalc) ? undefined : _montoCalc,   // referencia de lo que daba el sistema
    estado:       'Pendiente',
    vence:        venceStr,
    fechaPago:    '',
    obs,
...
// ── DIFERENCIAS A FACTURAR (ALTO 2) ───────────────────────────────────────────
// Cuando se corrige un precio de OS con vigencia RETROACTIVA, las atenciones de un mes que
// ya se facturó cambian de total, pero la factura emitida quedó por el monto viejo. Estas
// funciones detectan, por (mes, OS), la diferencia entre lo YA facturado (facturas emitidas)
// y el total AJUSTADO actual, para que el usuario emita una factura COMPLEMENTARIA por lo
// que falta. El reprecio NO se bloquea: se avisa la diferencia y el usuario la registra.
const _EST_FACT_EMITIDA = ['Pendiente', 'Pagada', 'Vencida'];   // 'Preliquidada' = borrador, no cuenta

function _facturasEmitidasDe(os, mesLabel) {
  return (DB.facturas || []).filter(f =>
    f.dest === os && f.mes === mesLabel && _EST_FACT_EMITIDA.includes(f.estado));
}

// Devuelve, por (mes, OS) YA facturado, la diferencia entre el total ajustado actual y lo
// facturado. Solo incluye diferencias relevantes (≥ $1). osFiltro y vigenciaDesde opcionales.
function _diferenciasAFacturar(osFiltro, vigenciaDesde) {
  const vigMes = vigenciaDesde ? vigenciaDesde.slice(0, 7) : null;   // 'YYYY-MM'
  const grupos = {};   // 'os||YYYY-MM' → registros
  (DB.registros || []).forEach(r => {
    if (!r.fecha || r.os === 'Particular' || r.os === 'CEMEPLA') return;
    if (osFiltro && r.os !== osFiltro) return;
    const mes = r.fecha.slice(0, 7);
    if (vigMes && mes < vigMes) return;
    (grupos[r.os + '||' + mes] = grupos[r.os + '||' + mes] || []).push(r);
  });
  const out = [];
  Object.keys(grupos).forEach(key => {
    const [os, mes] = key.split('||');
    const mesLabel = getMesLabel(mes);
    const emitidas = _facturasEmitidasDe(os, mesLabel);
    if (emitidas.length === 0) return;   // todavía no se facturó → no hay diferencia a emitir
    const t = totalesOS(grupos[key], os);
    const yaFacturado   = Math.round(emitidas.reduce((s, f) => s + (f.monto || 0), 0) * 100) / 100;
    const totalAjustado = Math.round(t.aFacturar * 100) / 100;
    const diferencia    = Math.round((totalAjustado - yaFacturado) * 100) / 100;
    if (Math.abs(diferencia) < 1) return;   // sin diferencia relevante (ignora centavos)
    out.push({ os, mes, mesLabel, yaFacturado, totalAjustado, diferencia, nFacturas: emitidas.length });
  });
  return out.sort((a, b) => b.mes.localeCompare(a.mes) || a.os.localeCompare(b.os));
}

// Abre el modal de factura PREFIJADO con la diferencia de (os, mes), para emitir la
// complementaria. Reusa el flujo normal: el usuario pone el nº real y la fecha en que la
// hizo, y al confirmar queda como una factura Pendiente que cierra la diferencia.
function registrarFacturaComplementaria(os, mes) {
  const mesLabel = getMesLabel(mes);
  const regs = (DB.registros || []).filter(r =>
    r.os === os && (r.fecha || '').startsWith(mes) && r.os !== 'Particular' && r.os !== 'CEMEPLA');
  const t = totalesOS(regs, os);
  const emitidas = _facturasEmitidasDe(os, mesLabel);
  const sum = campo => emitidas.reduce((s, f) => s + (f[campo] || 0), 0);
  const r2 = n => Math.round(n * 100) / 100;
  const diffExento  = r2(t.netoExentoAFacturar  - sum('netoExento'));
  const diffGravado = r2(t.netoGravadoAFacturar - sum('netoGravado'));
  const diffIVA     = r2(t.ivaAFacturar         - sum('ivaGravado'));
  const diffTotal   = r2(diffExento + diffGravado + diffIVA);
  if (diffTotal <= 0) {
    showToast(`No hay diferencia positiva para facturar en ${os} · ${mesLabel}.`);
    return;
  }
  // cant = prestaciones "nuevas" no tiene sentido en una complementaria: se informa el total del mes
  const cant = regs.reduce((s, r) => s + (r.cantidad || 0), 0);
  abrirConfirmarFactura(os, mes, mesLabel, diffTotal, Math.max(0, diffExento), Math.max(0, diffGravado), Math.max(0, diffIVA), cant, 0);
  const obsEl = document.getElementById('cf-obs');
  if (obsEl) obsEl.value = `Complementaria — ajuste de precios ${mesLabel}`;
}
...
function abrirConfirmarCobro(facturaId) {
  ...
  _actualizarDiferenciaCobro();
  openModal('modal-confirmar-cobro');
}

// Muestra en vivo la diferencia entre lo facturado y lo que se está por cobrar.
function _actualizarDiferenciaCobro() {
  const el = document.getElementById('cobro-diferencia');
  if (!el) return;
  const id = parseInt(document.getElementById('cobro-factura-id').value);
  const f = DB.facturas.find(x => x.id === id);
  const cobrado = parseFloat(document.getElementById('cobro-monto').value);
  if (!f || isNaN(cobrado)) { el.innerHTML = ''; return; }
  const dif = Math.round((cobrado - f.monto) * 100) / 100;
  if (Math.abs(dif) < 0.005)      el.innerHTML = `<span style="color:var(--success);">✓ Coincide con lo facturado (${fmt(f.monto)})</span>`;
  else if (dif < 0)               el.innerHTML = `<span style="color:var(--danger);">⚠️ Cobraron ${fmt(-dif)} MENOS que lo facturado (facturado ${fmt(f.monto)})</span>`;
  else                            el.innerHTML = `<span style="color:var(--warn);">Cobraron ${fmt(dif)} de MÁS que lo facturado (facturado ${fmt(f.monto)})</span>`;
}
...
function confirmarCobro() {
  ...
  const f = DB.facturas.find(x => x.id === id);
  if (!f) return;
  // Anti doble-cobro: si ya está pagada (doble click, o el modal quedó abierto), no crear
  // otro pago ni otro ingreso en Caja. Antes esto duplicaba la plata cobrada.
  if (f.estado === 'Pagada') { showToast('⚠️ Esta factura ya figura como cobrada.'); closeModal('modal-confirmar-cobro'); return; }
  const monto = parseFloat(document.getElementById('cobro-monto').value);
  if (isNaN(monto) || monto <= 0) { showToast('⚠️ Ingresá un monto cobrado válido (mayor a 0).'); return; }
  ...
  // Diferencia entre lo facturado y lo cobrado (positivo = cobraron de más).
  const diferencia = Math.round((monto - f.monto) * 100) / 100;

  f.estado = 'Pagada';
  f.fechaPago = fecha;
  f.montoCobrado = monto;          // lo realmente cobrado (puede diferir del facturado)
  f.diferenciaCobro = diferencia;  // 0 si coincide; ≠0 si hubo diferencia
  if (obs) f.obs = obs;

  DB.pagosRecibidos.push({
    id: DB.nextId++, facturaId: id, num: f.num, os: f.dest,
    fecha, monto, facturado: f.monto, diferencia, medioPago: medio, referencia: ref,
  });
  ...
  marcarCambios('facturas'); marcarCambios('pagosRecibidos'); marcarCambios('movimientos');
  showToast(Math.abs(diferencia) < 0.005
    ? `✓ Cobro de ${f.num} confirmado — ${fmt(monto)}`
    : `✓ Cobro de ${f.num} — ${fmt(monto)} · ⚠️ ${diferencia < 0 ? 'faltan' : 'sobran'} ${fmt(Math.abs(diferencia))} vs. lo facturado`);
  renderCobranzas();
  renderOS();
}
...
function guardarEdicionFactura() {
  ...
  cerrarPanelOSDetalle();   // si se editó desde el detalle de la OS, evitar dejarlo con datos viejos
  renderCobranzas();
  renderOS();
  renderFinanzas();
...
function eliminarFactura() {
  ...
  closeModal('modal-editar-factura');
  cerrarPanelOSDetalle();   // el panel mostraba esta presentación: cerrarlo tras borrarla
  renderCobranzas();
  renderOS();
  renderFinanzas();
...
function renderFinanzas() {
  // ── KPIs dinámicos ──────────────────────────────────────────────────────────
  // Mes actual en hora Argentina (hoyISO), no UTC: con new Date().toISOString() el
  // último día del mes, de noche, saltaba al mes siguiente y el KPI caía a $0.
  ...
  if (cajaTbody) {
    ...
    cajaTbody.innerHTML = movsSaldo.length === 0
      ? `<tr><td colspan="7" style="text-align:center;padding:20px;color:var(--text3);">Sin movimientos de banco</td></tr>`
      : movsSaldo.map(m => `
        <tr>
          <td>${m.fecha}</td>
          <td>${m.desc}</td>
          ...
          <td style="white-space:nowrap; text-align:right;">
            <button class="btn-icon" title="Editar" onclick="editarMovimiento(${m.id})" style="background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px;">✏️</button>
            <button class="btn-icon" title="Eliminar" onclick="eliminarMovimiento(${m.id})" style="background:none;border:none;cursor:pointer;font-size:14px;padding:2px 4px;">🗑️</button>
          </td>
        </tr>`).join('');
  }
