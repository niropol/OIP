...
function atResetModal() {
  ...
  const prestOtra = document.getElementById('at-prest-otra'); if (prestOtra) { prestOtra.value = ''; prestOtra.style.display = 'none'; }
...
function atSelCategoria(cat) {
  ...
  const prests = (cat === 'consulta' ? getConsultasDeOS(AT.os)
    ...
    : getPrestacionesSinConsulta(AT.os)).sort((a, b) => cmpAlfa(a.desc, b.desc));
  ...
  // Opción para cargar una prestación NO convenida (nombre libre + monto a mano), sin
  // agregarla al nomenclador de la OS. Toma el IVA por defecto de la OS (después se puede togglear).
  sel.innerHTML = optsPrest + `<option value="__otra__" data-val="0" data-exenta="${exAttr0}">➕ Otra prestación (nombre + valor a mano)…</option>`;
  ...
  const otraInp = document.getElementById('at-prest-otra');
...
function atOnPrestChange() {
  ...
  // "Otra prestación (a presupuesto)": mostrar el campo de nombre libre y dejar el valor en
  // blanco para que se cargue a mano. Cualquier otra opción: ocultar ese campo.
  const esOtra = opt?.value === '__otra__';
  const otraInp = document.getElementById('at-prest-otra');
  if (otraInp) { otraInp.style.display = esOtra ? '' : 'none'; if (esOtra) setTimeout(() => otraInp.focus(), 0); }

  const montoInp = document.getElementById('at-monto');
  if (montoInp) montoInp.value = esOtra ? '' : val;

  const badge = document.getElementById('at-iva-badge');
  const ivaHid = document.getElementById('at-iva-val');
  const ivaPct = (Math.round(pctIVAForOS(AT.os) * 1000) / 10) + '%';   // alícuota de la OS (data-driven)
  if (badge) {
...
function guardarAtencion() {
  ...
  const sel      = document.getElementById('at-prestacion-sel');
  const opt      = sel?.options[sel.selectedIndex];
  // "Otra prestación (a presupuesto)": nombre libre tipeado por el usuario, sin código de
  // nomenclador (no se agrega al catálogo de la OS; es puntual). Requiere nombre.
  const esOtra   = opt?.value === '__otra__';
  const rawText  = opt ? opt.text : 'Consulta vestida';
  let prestDesc  = esOtra
    ? (document.getElementById('at-prest-otra')?.value.trim() || '')
    : (rawText.includes(' — ')
        ? rawText.split(' — ')[0].replace(/^\[.*?\]\s*/, '').trim()
        : rawText.replace(/^\[.*?\]\s*/, '').trim() || 'Consulta vestida');
  if (esOtra && !prestDesc) {
    showToast('⚠️ Escribí el nombre de la prestación.');
    const inp = document.getElementById('at-prest-otra'); if (inp) inp.focus();
    return;
  }
  const montoRaw = parseFloat(document.getElementById('at-monto').value) || 0;
  // Código de nomenclador de la prestación elegida (para repreciar). "Otra" no tiene código.
  const _pNom = esOtra ? null : (DB.prestaciones || []).find(p => p.os === AT.os && p.desc === prestDesc);
  ...
  // Comentario/observación de la atención. Antes se escribía en el campo y se perdía (no se
  // guardaba). Ahora se conserva en el registro y viaja a la presentación de la OS, sobre
  // todo útil en prácticas/cirugías (una aclaración para la obra social).
  const comentario = document.getElementById('at-obs')?.value.trim() || '';
